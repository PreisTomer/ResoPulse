...
export function buildHealthySection(
  h: CellParamSnapshot,
  isDbl: boolean,
  mediumName: string,
): string[] {
  ...
  const lines: string[] = [
    ...
    fld(
      "Membrane conductivity σm",
      `${sigmaMem.toExponential(1)} ${UNIT.S_PER_M}  [lipid bilayer leakage, Gascoyne 2002]`,
    ),
    ...
    fld(
      "Lysis threshold Vm,thr",
      `${h.thresholdVoltage} ${UNIT.V}  [base, at 37°C]`,
    ),
  ...
  if (isDbl && h.nuclearRadius) {
    lines.push(
      ...
      fld(
        "Nuclear mem. thickness",
        `${h.nuclearMembraneThickness ?? "n/a"} ${UNIT.NM}`,
      ),
      fld("Nuclear mem. εr", `${h.nuclearMembraneEps ?? "n/a"}`),
      fld(
        "Nucleoplasm σn",
        `${h.nucleoplasmConductivity ?? "n/a"} ${UNIT.S_PER_M}`,
      ),
    );
  }
  return lines;
...
export function buildTargetSection(
  ...
  if (isRes && t.resonantFreqGHz) {
    ...
    lines.push(
      fld(
        ...
        `${fResGHz} ${UNIT.GHZ}  (${fResMHz} ${UNIT.MHZ})`,
      ),
      fld("Quality factor Q", `${qNominal}  [range: ${qMin} - ${qMax}]`),
      fld(
        "Resonant threshold E_thr",
        `${t.resonantThresholdVcm?.toFixed(0) ?? ", "} ${UNIT.V_PER_CM}`,
      ),
      fld(
        "f_res uncertainty",
        `±${t.resonantFreqUncertaintyPct?.toFixed(0) ?? ", "}%`,
      ),
    ...
    lines.push(
      fld(
        "Lysis threshold Vm,thr",
        `${t.thresholdVoltage} ${UNIT.V}  [base, at 37°C]`,
      ),
      fld("Time constant τ", `${tau} ${UNIT.NS}  [medium: ${mediumName}]`),
  ...
  if (isDbl && t.nuclearRadius) {
    lines.push(
      ...
      fld(
        "Nuclear mem. thickness",
        `${t.nuclearMembraneThickness ?? "n/a"} ${UNIT.NM}`,
      ),
      fld("Nuclear mem. εr", `${t.nuclearMembraneEps ?? "n/a"}`),
      fld(
        "Nucleoplasm σn",
        `${t.nucleoplasmConductivity ?? "n/a"} ${UNIT.S_PER_M}`,
      ),
    );
  }
  if (t.category === "virus") {
...
function classifyDR(ratio: number): string {
  if (ratio >= THRESHOLDS.DISRUPTION_WARN)
    return "LYSIS-ARMED, irreversible EP zone (sustained → lysis)";
  if (ratio >= THRESHOLDS.HEALTHY_APPROACHING)
    return "Reversible EP, membrane transiently permeabilised, recoverable";
  if (ratio >= THRESHOLDS.NOURISHING)
    return "Approaching, sub-EP membrane stress, ion-channel perturbation";
  if (ratio >= THRESHOLDS.VIBRATING_MIN)
    return "Nourishing, sub-threshold biomodulation window";
...
function computeLysisDelayMs(entry: LogEntry): number {
  if (
    (entry.waveform === "pulsed" || entry.waveform === "hfire") &&
    entry.pulseWidthNs &&
    entry.dutyCycle
  ) {
...
export function buildEntryMethodsText(
  entry: LogEntry,
  sessionName: string,
  sampleDescription = "",
  ...
  if (!entry.healthySnap || !entry.targetSnap) {
    ...
    const medName =
      entry.medium in MEDIA
        ? MEDIA[entry.medium as MediumKey].name
        : entry.medium;
    const text = [
  ...
  const sigmaCorr =
    baseS !== undefined && tempCoeff !== undefined
  ...
  const protocolLines: string[] = [
    ...
    fld(
      "Conductivity σe (eff)",
      `${sigE.toFixed(4)} ${UNIT.S_PER_M}  [T-corrected]`,
    ),
    fld("σe correction", sigmaCorr),
    fld(
      "Medium permittivity εr",
      `${medEpsR ?? "n/a"}  [used in DEP Clausius-Mossotti]`,
    ),
    ...(phi > 0
      ? [
          fld(
            "Cell packing fraction φ",
            `${phi.toFixed(3)}  [Maxwell-Garnett σe correction applied]`,
          ),
        ]
      : []),
    ...
    fld(
      "Waveform",
      `${(entry.waveform ?? "cw").toUpperCase()}${isHFire ? "  [H-FIRE bipolar burst, Vth × 1.75 — model parameter, see PHYSICAL MODEL]" : ""}`,
    ),
    ...(isPulsedOrHFire
      ? [
    ...
    fld(
      "Cell orientation θ",
      `${entry.orientationDeg ?? 0}°  (cos θ = ${cosTheta.toFixed(3)})`,
    ),
    ...(perfRate > 0
      ? [
          fld(
            "Perfusion rate ω_b",
            `${perfRate.toFixed(2)} mL/(g·min)  [Pennes perfusion term active]`,
          ),
        ]
  ...
  const formatDC = (dc: number): string =>
    `(×dc = ${dc >= 0.001 ? (dc * 100).toFixed(1) + "%" : dc.toExponential(2)})`;

  // Healthy cell block
  const wf =
    entry.waveform === "pulsed" || entry.waveform === "hfire" ? 1.0 : 0.5;
  const hSAR = computeSAR(h, entry.fieldVcm, sigmaE_val, wf);
  const hSARAvg = hSAR * (entry.dutyCycle ?? 1);
  const hPEF =
    isPulsedSch && entry.pulseWidthNs && h.fc > 0
      ? computePulseStepResponse(computeTau(h, sigmaE_val), entry.pulseWidthNs)
      : null;
  const hDRState = classifyDR(entry.healthyRatio);
  const hVthEff =
    h.thresholdVoltage *
    Math.max(0.7, 1 - TEMP_EP_COEFF * Math.max(0, entry.healthyTemp - 37));
  ...
  const healthyLines: string[] = [
    ...
    fld(
      "Lysis Vth (effective)",
      `${(hVthEff * 1000).toFixed(1)} ${UNIT.MV}  [base ${(h.thresholdVoltage * 1000).toFixed(0)} mV corrected at ${entry.healthyTemp}°C  ×max(0.70, 1−0.003×(T−37))]`,
    ),
    fld(
      "Disruption ratio DR",
      `${(entry.healthyRatio * 100).toFixed(2)}%  [single-cell nominal radius]  ${hDRState}`,
    ),
    ...(hPEF !== null
      ? [
          fld(
            "Pulse env. factor PEF",
            `${hPEF.toFixed(4)}  [1−exp(−t_p/τ)  t_p=${entry.pulseWidthNs}ns  τ=${(1e9 / (TWO_PI * h.fc * 1000)).toFixed(0)}ns]`,
          ),
        ]
      : []),
    fld(
      "SAR (instantaneous)",
      `${hSAR.toFixed(3)} ${UNIT.W_PER_KG}  [σi·α²·E²·wf/ρ  ρ=${h.density} kg/m³]`,
    ),
    fld(
      "SAR (time-averaged)",
      `${hSARAvg.toFixed(4)} ${UNIT.W_PER_KG}  ${formatDC(entry.dutyCycle ?? 1)}`,
    ),
    fld(
      "Steady-state T_ss",
      `${entry.healthyTemp} ${UNIT.DEG_C}  [37 + SAR_eff/((λ_N+λ_p)×cp)  cp=${h.specificHeatCapacity} J/(kg·K)]`,
    ),
    ...(isDbl && entry.healthyNuclearVm !== undefined
      ? [fld("Nuclear Vm", `${entry.healthyNuclearVm} ${UNIT.MV}`)]
      : []),
    ...(!isRes && entry.depHealthyK !== undefined
      ? (() => {
          ...
          return [
            fld(
              "Re[K] (DEP)",
              `${depKDisplayFull(entry.depHealthyK)}  [${entry.depHealthyK > 0 ? "pDEP: attracted to field maxima" : "nDEP: repelled from field maxima"}]  f_cross = ${xover ? xover.toFixed(1) + " " + UNIT.KHZ : "none in range"}`,
            ),
          ];
        })()
      : []),
  ...
  const targetLines: string[] = [
    ...
    ...(isRes
      ? [
          fld(
            "Vm (Schwan)",
            `< 0.01 ${UNIT.MV}  [Vm → 0 at f_res; ωτ >> 1; disruption governed by Lorentzian DR below]`,
          ),
        ]
      : [fld("Vm (Schwan)", `${entry.targetVm} ${UNIT.MV}`)]),
    fld(
      "Lysis Vth (effective)",
      `${(tVthEff * 1000).toFixed(1)} ${UNIT.MV}  [base ${(t.thresholdVoltage * 1000).toFixed(0)} mV corrected at ${entry.targetTemp}°C  ×max(0.70, 1−0.003×(T−37))]`,
    ),
    fld(
      "Disruption ratio DR",
      `${(entry.targetRatio * 100).toFixed(2)}%  [single-cell nominal radius]  ${tDRState}`,
    ),
    ...(tPEF !== null
      ? [
          fld(
            "Pulse env. factor PEF",
            `${tPEF.toFixed(4)}  [1−exp(−t_p/τ)  t_p=${entry.pulseWidthNs}ns  τ=${(1e9 / (TWO_PI * t.fc * 1000)).toFixed(0)}ns]`,
          ),
        ]
      : []),
    fld(
      "SAR (instantaneous)",
      `${tSAR.toFixed(3)} ${UNIT.W_PER_KG}  [σi·α²·E²·wf/ρ  ρ=${t.density} kg/m³]`,
    ),
    fld(
      "SAR (time-averaged)",
      `${tSARAvg.toFixed(4)} ${UNIT.W_PER_KG}  ${formatDC(entry.dutyCycle ?? 1)}`,
    ),
    fld(
      "Steady-state T_ss",
      `${entry.targetTemp} ${UNIT.DEG_C}  [37 + SAR_eff/((λ_N+λ_p)×cp)  cp=${t.specificHeatCapacity} J/(kg·K)]`,
    ),
    ...(isDbl && entry.targetNuclearVm !== undefined
      ? [fld("Nuclear Vm", `${entry.targetNuclearVm} ${UNIT.MV}`)]
      : []),
    ...(!isRes && entry.depTargetK !== undefined
      ? (() => {
          ...
          return [
            fld(
              "Re[K] (DEP)",
              `${depKDisplayFull(entry.depTargetK)}  [${entry.depTargetK > 0 ? "pDEP: attracted to field maxima" : "nDEP: repelled from field maxima"}]  f_cross = ${xover ? xover.toFixed(1) + " " + UNIT.KHZ : "none in range"}`,
            ),
          ];
        })()
      : []),
  ...
  const inWindow =
    entry.targetRatio >= THRESHOLDS.DISRUPTION_WARN &&
    entry.healthyRatio < THRESHOLDS.HEALTHY_APPROACHING;
  ...
  const assessmentLines: string[] = [
    fld(
      ...
      `${entry.selectivity.toFixed(3)}×  ${selClass}`,
    ),
    fld(
      "Vm selectivity",
      `${vmSelRatio.toFixed(3)}×  (Vm_T / Vm_H, orientation-independent)`,
    ),
    fld(
      "Protocol window",
      inWindow
    ...
    ...(entry.event === "lysis"
      ? [
          fld(
            "Lysis delay (modelled)",
            `${lysisDelayMs.toFixed(0)} ms  [max(200ms, min(30s, N×t_p/dc))${isPulsedOrHFire ? `  N=${entry.lysisNPulses ?? 1} × ${entry.pulseWidthNs}ns / ${(entry.dutyCycle ?? 1).toExponential(2)}` : "  CW default 2500ms"}]`,
          ),
        ]
      : []),
    fld("Event", entry.event),
...
export function buildCsvText(
  ...
  const distinctSessions = [
    ...new Set(entries.map((e) => e.sessionName ?? sessionName)),
  ];
  const meta = [
  ...
  const rows = entries.map((e) => [
    ...
    e.depHealthyK ?? "",
    e.depTargetK ?? "",
    e.depHealthyCrossoverKHz ?? "",
    e.depTargetCrossoverKHz ?? "",

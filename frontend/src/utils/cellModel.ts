...
type ResonanceCell = Pick<
  CellConfig,
  "resonantFreqGHz" | "resonantThresholdVcm"
>;
type CellCategoryValue = "mammalian" | "bacteria" | "virus";
type WaveformValue = "cw" | "pulsed" | "hfire";

export function isResonanceCapableCategory(
  category: CellCategoryValue,
): boolean {
  return (
    category === CELL_CATEGORY.BACTERIA || category === CELL_CATEGORY.VIRUS
  );
}

export function hasResonanceConfig(
  cell: ResonanceCell,
...
export function effectiveElectroporationThreshold(
  ...
  return (
    tempCorrectedVth(nominalVth, tempC) * getHfireThresholdMultiplier(waveform)
  );
}

...
type MembraneCellLike = Pick<
  CellConfig,
  "dielectricConstant" | "membraneThickness"
>;
type TauCellLike = MembraneCellLike &
  Pick<CellConfig, "radius" | "conductivity">;
...
export function membraneCm(cell: MembraneCellLike): number {
  return (
    (cell.dielectricConstant * EPSILON_0) / (cell.membraneThickness * NM_TO_M)
  );
}
...
export function computeSAR(
  ...
  return (
    (cell.conductivity * alpha ** 2 * E_si ** 2 * waveformFactor) / cell.density
  );
}
...
function cadd(a: Cpx, b: Cpx): Cpx {
  return [a[0] + b[0], a[1] + b[1]];
}
function csub(a: Cpx, b: Cpx): Cpx {
  return [a[0] - b[0], a[1] - b[1]];
}
...
function cscale(a: Cpx, s: number): Cpx {
  return [a[0] * s, a[1] * s];
}
...
export function computeDepCmReal(
  ...
  if (omega < 1)
    return (cell.conductivity - sigma_e) / (cell.conductivity + 2 * sigma_e);

  // Complex permittivities: ε*(ω) = ε_r·ε₀ − j·σ/ω
  const epsCyto: Cpx = [
    EPSILON_R_CYTOPLASM * EPSILON_0,
    -cell.conductivity / omega,
  ];
...
export function computeDepCrossoverKHz(
  ...
  const fLo = 1,
    fHi = 10_000_000; // kHz: 1 kHz to 10 GHz
...
export function computePulseStepResponse(
  tau_s: number,
  pulseWidthNs: number,
): number {
...
export function computeSkinDepthMm(
  freqKHz: number,
  sigma_e: number,
  epsilon_r = 80,
): number {
...
export function computeResonantDisruption(
  ...
  return (
    (fieldVcm / thresholdVcm) *
    computeResonantLineshape(resonantFreqGHz, Q, freqHz)
  );
}
...
export function tempCorrectedVth(nominalVth: number, tempC: number): number {
  const correction = Math.max(
    TEMP_EP_CLAMP_MIN,
    1 - TEMP_EP_COEFF * Math.max(0, tempC - BODY_TEMP_C),
  );
...
export function computeLysisProbability(
  dr: number,
  center: number,
  slope: number,
): number {

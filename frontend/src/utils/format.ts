...
export function formatFreqKHz(khz: number, decimals = 2): string {
  if (khz >= KHZ_PER_GHZ)
    return `${(khz / KHZ_PER_GHZ).toFixed(decimals)} ${UNIT.GHZ}`;
  if (khz >= KHZ_PER_MHZ)
    return `${(khz / KHZ_PER_MHZ).toFixed(decimals)} ${UNIT.MHZ}`;
  return `${Math.round(khz)} ${UNIT.KHZ}`;
}

// Split frequency [kHz] into { value, unit } for two-column layout
export function splitFreqKHz(
  khz: number,
  decimals = 2,
): { value: string; unit: string } {
  if (khz >= KHZ_PER_GHZ)
    return { value: (khz / KHZ_PER_GHZ).toFixed(decimals), unit: UNIT.GHZ };
  if (khz >= KHZ_PER_MHZ)
    return { value: (khz / KHZ_PER_MHZ).toFixed(decimals), unit: UNIT.MHZ };

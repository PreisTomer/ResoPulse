...
describe('cellModel helpers', () => {
  ...
  const incompleteCell = { resonantFreqGHz: 0.7, resonantThresholdVcm: undefined }
  ...
  it('applies the H-FIRE threshold multiplier only in H-FIRE mode', () => {
    ...
    expect(getHfireThresholdMultiplier('hfire')).toBe(H_FIRE_THRESHOLD_MULTIPLIER)
  })

  it('uses waveform-aware electroporation thresholds', () => {
    expect(effectiveElectroporationThreshold(1, 37, 'cw')).toBeCloseTo(1, 6)
    expect(effectiveElectroporationThreshold(1, 37, 'hfire')).toBeCloseTo(H_FIRE_THRESHOLD_MULTIPLIER, 6)
  })

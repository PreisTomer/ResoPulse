...
export function tipNuclearSection(params: {
  ...
  const peakT = hasTargetNucleus
    ? `  fc(T, nuc) = <span class="tip-val">${formatTooltipFrequency(targetFpeakKHz)}</span>  (${targetLabel})`
    : `  ${targetLabel}: no nuclear params`;
  const peakH = hasHealthyNucleus
    ? `  fc(H, nuc) = <span class="tip-val">${formatTooltipFrequency(healthyFpeakKHz)}</span>  (${healthyLabel})`
    : `  ${healthyLabel}: no nuclear params`;
...
export function tipTargetBar(params: {
  ...
  const warn =
    getDisruptionWarningState(targetRatio) !== "none"
...
export function tipHealthyBar(params: {
  ...
  const { pct, isResonanceTarget, healthyVmMv, thresholdMv, healthyRatio } =
    params;
  if (isResonanceTarget) {
...
Ref: Tsen et al. (2007)`;
  ...
  const status =
    healthyRatio < THRESHOLDS.HEALTHY_APPROACHING

...
export function tipVm(opts: {
  ...
  const pulsedNote =
    waveform === WAVEFORM.PULSED || waveform === WAVEFORM.H_FIRE
...
export function tipTemp(opts: {
  ...
  const warnLine = buildThermalWarningLine(
    resolveThermalWarningLevel({ tempVaporizing, tempDenaturing, tempWarning }),
    {
      ...
      denaturing:
        "⚠ ≥60°C, protein denaturation (collagen ~60°C, albumin ~68°C), reduce duty cycle / field",
      hyperthermic:
        "⚠ ≥42°C, hyperthermic damage onset (IAHT threshold), monitor closely",
    },
  );
...
export function tipState(opts: {
  ...
  const labels: Record<string, string> = {
    ...
    [CELL_STATE.NOURISHING]:
      '<span class="tip-ok">nourishing, sub-threshold Ca²⁺ stimulation window (DR 8-45%)\nMembrane intact · PIEZO1 + voltage-gated Ca²⁺ channels activated\nOptimal biomodulation at DR ≈ 20-40% of lysis threshold</span>',
    [CELL_STATE.APPROACHING]:
      '<span class="tip-warn">⚠ approaching, membrane stress OR T ≥ 42°C · ion channel perturbation onset</span>',
    [CELL_STATE.REV_EP]:
      '<span class="tip-warn">⚡ reversible EP window (50-85%), membrane transiently permeabilized each pulse.\nPores open and re-seal after the field is removed.\nThis is the drug/gene delivery window, cells survive.\nSustained or increasing field progresses to irreversible lysis.</span>',
    [CELL_STATE.CRITICAL]:
      '<span class="tip-warn">⚡ critical, Vm >85% threshold OR T ≥ 60°C (protein denaturation) · reduce field / duty cycle immediately</span>',
    [CELL_STATE.VIBRATING]:
      '<span class="tip-warn">⚡ LYSIS ARMED, Vm >85% of threshold · irreversible electroporation imminent</span>',
    [CELL_STATE.LYSING]:
      '<span class="tip-warn">lysing, irreversible membrane disruption in progress</span>',
...
export function tipAcousticVm(opts: {
  ...
  const basisNote =
    experimentalBasis === EXPERIMENTAL_BASIS.SPECULATIVE
...
export function tipDisruption(opts: {
  ...
  const revEpNote =
    ...
    disruptionRatio >= THRESHOLDS.HEALTHY_APPROACHING &&
    disruptionRatio < THRESHOLDS.DISRUPTION_WARN

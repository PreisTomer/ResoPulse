...
describe('sharedTooltip helpers', () => {
  ...
  it('resolves thermal warning levels in severity order', () => {
    expect(resolveThermalWarningLevel({ tempWarning: true })).toBe('hyperthermic')
    expect(resolveThermalWarningLevel({ tempDenaturing: true, tempWarning: true })).toBe('denaturing')
    expect(resolveThermalWarningLevel({ tempVaporizing: true, tempDenaturing: true })).toBe('vaporizing')
    expect(resolveThermalWarningLevel({})).toBe(null)
  })

  it('builds thermal warning lines from shared severity state', () => {
    expect(buildThermalWarningLine('hyperthermic', {
  ...
  it('formats Vm and threshold summary lines', () => {
    expect(formatVmThresholdLine('120 mV', '850 mV')).toBe('Vm = <span class="tip-val">120 mV</span>  ·  Threshold = 850 mV')
  })
})
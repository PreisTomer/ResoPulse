...
<template>
  <div class="freq-chart">
    ...
    <div class="freq-chart__header">
      <span class="freq-chart__title" v-tip="$t('chart.tipMainTitle')">{{
        $t("chart.title")
      }}</span>
      <ChartLegend />
    </div>
...
import {
  ...
  formatHz,
  computeVmCurve,
  computeNuclearVmCurve,
  computeDepCurve,
  computeSelCurve,
  computeOptimalFreqHz,
  computeUncBand,
  sigmaUncPct,
} from "./lib/chartCompute";
import type { TooltipData } from "./lib/chartCompute";
...
export default defineComponent({
  ...
  watch: {
    ...
    "cellStore.effectiveSigmaE": {
      handler() {
        this.updateChart();
      },
    },
    "cellStore.cosThetaFactor": {
      handler() {
        this.updateChart();
      },
    },
    "cellStore.currentBroadcastFrequency": {
      handler() {
        if (!this._isDragging) this.updateCursor();
      },
    },
    "cellStore.doubleShellEnabled": {
      handler() {
        this.updateChart();
      },
    },
  ...
  mounted() {
    ...
    this._resizeObserver = observeChartResize(el, () => {
      this.initChart();
      this.updateChart();
    });
  },
  ...
  methods: {
    ...
    initChart() {
      ...
      const { totalW, totalH, chartW, chartH } = measureChartBox(
        container,
        MARGIN,
        640,
        260,
      ...
      const svgEl = createSvgRoot(container, totalW, totalH);
      ...
      g.append("g")
        .attr("class", "x-axis")
        .attr("transform", `translate(0,${this._chartH})`);
      g.append("g").attr("class", "y-axis");
      g.append("g")
        .attr("class", "y-right-axis")
        .attr("transform", `translate(${this._chartW},0)`);
      // DEP Re[K] axis - further right, only shown in Schwan mode
      g.append("g")
        .attr("class", "y-dep-axis")
        .attr("transform", `translate(${this._chartW + 90},0)`);
    ...
    updateChartResonance() {
      ...
      const lorCurve = freqPoints.map((hz) => ({
        hz,
        dr: computeResonantLineshape(t.resonantFreqGHz!, Q, hz) * (E / Ethr),
      }));
      const lorQmin = freqPoints.map((hz) => ({
        hz,
        dr: computeResonantLineshape(t.resonantFreqGHz!, Qmin, hz) * (E / Ethr),
      }));
      const lorQmax = freqPoints.map((hz) => ({
        hz,
        dr: computeResonantLineshape(t.resonantFreqGHz!, Qmax, hz) * (E / Ethr),
      }));
      ...
      if (!tickVals.some((v) => Math.abs(v - f0_hz) / f0_hz < 0.05))
        tickVals.push(f0_hz);
      ...
      g.select<SVGGElement>(".x-axis")
        .call(
          d3
            .axisBottom<number>(this._xScale!)
            .tickValues(tickVals)
            .tickFormat(formatResHz)
            .tickSize(4),
        )
      ...
      g.select<SVGGElement>(".grid-h")
        .call(
          d3
            .axisLeft<number>(this._yScale!)
            .ticks(5)
            .tickSize(-this._chartW)
            .tickFormat(() => ""),
        )
      ...
      if (Qmin !== Qmax) {
        const areaGen = d3
          .area<{ hz: number; drLow: number; drHigh: number }>()
          .x((d) => this._xScale!(d.hz))
      ...
      if (pct) {
        for (const f of [f0_hz * (1 - pct / 100), f0_hz * (1 + pct / 100)]) {
          if (f >= fMin && f <= fMax) {
            fcGroup
              .append("line")
              .attr("x1", this._xScale!(f))
              .attr("x2", this._xScale!(f))
              .attr("y1", 0)
              .attr("y2", this._chartH)
              .attr("stroke", C.amber)
              .attr("stroke-width", 0.75)
    ...
    updateChart() {
      ...
      const healthyCurve = computeVmCurve(
        this.cellStore.healthy,
        this.cellStore.fieldIntensity,
        sigma_e,
        cosTheta,
      );
      const targetCurve = computeVmCurve(
      ...
      // Auto-scale y
      const allVm = [
        ...healthyCurve.map((d) => d.vm),
        ...targetCurve.map((d) => d.vm),
      ];
      ...
      const yRightAxis = d3
        .axisRight<number>(this._yRightScale!)
        .ticks(4)
        .tickSize(3)
      ...
      const yDepAxis = d3
        .axisRight<number>(this._yDepScale!)
      ...
      const crossMarkers = [
        {
          fKhz: this.cellStore.depHealthyCrossoverKHz,
          color: C.primary,
          tag: this.$t("chart.depCrossH"),
        },
        {
          fKhz: this.cellStore.depHealthySecondCrossoverKHz,
          color: C.primary,
          tag: this.$t("chart.depCrossH2"),
        },
        {
          fKhz: this.cellStore.depTargetCrossoverKHz,
          color: C.danger,
          tag: this.$t("chart.depCrossT"),
        },
        {
          fKhz: this.cellStore.depTargetSecondCrossoverKHz,
          color: C.danger,
          tag: this.$t("chart.depCrossT2"),
        },
      ];
      ...
      const areaGen = d3
        .area<{ hz: number; vmLow: number; vmHigh: number }>()
        .x((d) => this._xScale!(d.hz))
      ...
      if (this.cellStore.doubleShellEnabled) {
        if (this.cellStore.healthy.nuclearRadius) {
          const hNucCurve = computeNuclearVmCurve(
            this.cellStore.healthy,
            this.cellStore.fieldIntensity,
            sigma_e,
            cosTheta,
        ...
        if (this.cellStore.target.nuclearRadius) {
          const tNucCurve = computeNuclearVmCurve(
            this.cellStore.target,
            this.cellStore.fieldIntensity,
            sigma_e,
            cosTheta,
      ...
      const thrData = [
        {
          label: this.$t("chart.thrH"),
          vm:
            tempCorrectedVth(
              this.cellStore.healthy.thresholdVoltage,
      ...
      const fcData = [
        {
          fc: computeFc(this.cellStore.healthy, sigma_e),
          color: C.primary,
          label: this.$t("chart.fcH"),
          tauNs: tauHNs,
        },
        {
          fc: computeFc(this.cellStore.target, sigma_e),
          color: C.danger,
          label: this.$t("chart.fcT"),
          tauNs: tauTNs,
        },
      ];
      ...
      const visibleFc = fcData
        .map(({ fc, color, label, tauNs }) => {
          ...
          const fcDisplay =
            fc >= 1000 ? `${(fc / 1000).toFixed(1)}M` : `${fc.toFixed(0)}k`;
          const tauDisplay =
            tauNs >= 1000
      ...
      const MIN_GAP_PX = 52;
      const anchors: ("middle" | "end" | "start")[] = visibleFc.map(
        () => "middle" as const,
      );
      ...
      const optHz = computeOptimalFreqHz(
        this.cellStore.healthy,
        this.cellStore.target,
        this.cellStore.fieldIntensity,
        sigma_e,
      ...
      if (optHz >= F_MIN_HZ && optHz <= F_MAX_HZ) {
        ...
        const hVmOpt = computeSchwan(
          this.cellStore.healthy,
          optKhz,
          this.cellStore.fieldIntensity,
          sigma_e,
        );
        const tVmOpt = computeSchwan(
          this.cellStore.target,
          optKhz,
          this.cellStore.fieldIntensity,
          sigma_e,
        ...
        optGroup
          ...
          .attr("stroke-opacity", 0.45);
        const optAnchor =
          ox < 60 ? "start" : ox > this._chartW - 60 ? "end" : "middle";
        const optTextX =
          ox < 60
            ? Math.max(4, ox)
            : ox > this._chartW - 60
              ? Math.min(this._chartW - 4, ox)
              : ox;
        optGroup
          .append("text")
          .attr("x", optTextX)
    ...
    updateCursor(overrideKhz?: number) {
      ...
      const label =
        freqKHz >= 1e6
          ? `${(freqKHz / 1e6).toFixed(2)} ${UNIT.GHZ}`
          : freqKHz >= 1000
            ? `${(freqKHz / 1000).toFixed(2)} ${UNIT.MHZ}`
            : `${freqKHz.toFixed(1)} ${UNIT.KHZ}`;
      const textEl = g.select<SVGTextElement>(".cursor-label");
      textEl.attr("x", x).text(label);

      const textWidth =
        (textEl.node()?.getComputedTextLength?.() ?? label.length * 5.5) + 8;
      g.select(".cursor-bg")
    ...
    onHover(event: MouseEvent) {
      ...
      const tVthEffMv =
        ...
        1000;
      const hDRPct =
        ((hVm * this.cellStore.pulseEnvelopeFactorHealthy) / hVthEffMv) * 100;
      const tDRPct =
        ((tVm * this.cellStore.pulseEnvelopeFactorTarget) / tVthEffMv) * 100;
      const selRatio = hVm > 0.01 ? tVm / hVm : 0;
      const inWindow =
        tDRPct >= THRESHOLDS.DISRUPTION_WARN * 100 &&
        hDRPct < THRESHOLDS.HEALTHY_APPROACHING * 100;
      const flipLeft = mx > (this._chartW ?? 0) * 0.55;
      const eps_r = MEDIA[this.cellStore.medium].permittivity;
      const depHealthyK = computeDepCmReal(
        this.cellStore.healthy,
        khz,
        sigma_e,
        eps_r,
      );
      const depTargetK = computeDepCmReal(
        this.cellStore.target,
        khz,
        sigma_e,
        eps_r,

      ...
      <div class="sel-panel__body">
        ...
        <div class="sel-panel__ratio-wrap" v-tip="tipSelectivity">
          ...
          <div class="sel-panel__ratio-labels">
            <span class="sel-panel__ratio-label">{{
              $t("selectivity.ratioLabel")
            }}</span>
            <span class="sel-panel__ti-label"
              >{{ $t("selectivity.vmSelLabel") }} {{ ICON.TIMES
              }}<span>{{
                vmSelectivityRatio >= 99
                  ? ICON.INFINITY
                  : vmSelectivityRatio.toFixed(2)
              }}</span></span
            >
          </div>
        </div>
        ...
        <div class="sel-panel__ws-row" v-tip="tipWindowScore">
          <span class="sel-panel__ws-label">{{
            $t("selectivity.windowScoreLabel")
          }}</span>
          <span class="sel-panel__ws-val" :class="windowScoreClass">{{
            windowScorePct
          }}</span>
          <span class="sel-panel__ws-formula">{{
            $t("selectivity.wsFormula")
          }}</span>
        </div>
        ...
        <div
          ...
          v-tip="tipTiRange"
        >
          <span class="sel-panel__ti-range-label">{{
            $t("selectivity.sigmaIRange")
          }}</span>
          <span class="sel-panel__ti-range-val">
            [{{ ICON.TIMES }}{{ tiRange.low.toFixed(2) }} - {{ ICON.TIMES
            }}{{
              tiRange.high >= 99 ? ICON.INFINITY : tiRange.high.toFixed(2)
            }}]
          </span>
        </div>
        ...
        <div
          v-if="smallCellNote"
          class="sel-panel__size-note"
          :class="{ 'sel-panel__size-note--inverted': selectionInverted }"
          v-tip="smallCellNoteTip"
        >
          <span class="sel-panel__size-note-icon">{{ ICON.WARNING }}</span>
          <span class="sel-panel__size-note-label">{{
            $t("selectivity.smallCellNoteLabel")
          }}</span>
          <span class="sel-panel__size-note-val"
            >R_T/R_H = {{ smallCellRadiusRatio }}</span
          >
          <span class="sel-panel__size-note-limit"
            >TI_DC ≤ {{ smallCellTiDcLimit }}</span
          >
        </div>
        ...
        <div
          class="sel-panel__orient-row"
          v-tip="$t('selectivity.tipOrientFrac')"
        >
          <span class="sel-panel__orient-label">{{
            $t("selectivity.orientFracLabel")
          }}</span>
        ...
        <div
          class="sel-panel__orient-row sel-panel__orient-row--popdist"
          v-tip="$t('selectivity.tipPopDist')"
        >
          <span class="sel-panel__orient-label">{{
            $t("selectivity.popDistLabel")
          }}</span>
          <div class="sel-panel__orient-vals">
...
import DepSection from "./components/DepSection.vue";
import VmSarGrid from "./components/VmSarGrid.vue";
import ResonanceInfo from "./components/ResonanceInfo.vue";
...
export default defineComponent({
  ...
  computed: {
    ...
    selectivity(): number {
      return this.cellStore.selectivityRatio;
    },

    isResonanceSectionVisible(): boolean {
      return this.isResonanceTarget && this.cellStore.isResonanceMode;
    },

    isResonanceTarget(): boolean {
      return isConfiguredResonanceTarget(
    ...
    toggleSubtitle(): string {
      ...
      const t = this.cellStore.targetDisruptionRatio,
        h = this.cellStore.healthyDisruptionRatio;
      const modeLabel = (() => {
        if (h >= THRESHOLDS.DISRUPTION_WARN)
          return this.$t("selectivity.modeAblative");
        if (
          t >= THRESHOLDS.DISRUPTION_WARN &&
          h < THRESHOLDS.HEALTHY_APPROACHING
        )
          return this.$t("selectivity.modeTherapeutic");
        if (t >= THRESHOLDS.DISRUPTION_WARN)
          return this.$t("selectivity.modeMarginal");
        if (t >= THRESHOLDS.HEALTHY_APPROACHING)
          return this.$t("selectivity.modeApproaching");
    ...
    selectivityClass(): string {
      if (this.selectivity >= THRESHOLDS.SEL_STRONG)
        return "sel-panel__ratio--strong";
      if (this.selectivity >= THRESHOLDS.SEL_MARGINAL)
        return "sel-panel__ratio--marginal";
      return "sel-panel__ratio--weak";
    },

    vmSelectivityRatio(): number {
      return safeRatio(
        this.cellStore.targetVm,
    ...
    tiRange(): { low: number; high: number } {
      return this.cellStore.tiUncertaintyRange;
    },

    showTiUncertainty(): boolean {
      return (
        !this.cellStore.isResonanceMode &&
        Math.abs(this.tiRange.high - this.tiRange.low) > 0.01
      );
    },
    ...
    windowScore(): number {
      return computeWindowScore(
        this.cellStore.targetLysisProbabilityRandom,
        this.cellStore.healthyLysisProbabilityRandom,
      );
    },

    windowScorePct(): string {
      return formatPct(this.windowScore);
    },
    ...
    windowScoreClass(): string {
      return `sel-panel__ws-val--${getWindowScoreTier(this.windowScore)}`;
    },
    ...
    tipWindowScore(): string {
      ...
      return this.$t("selectivity.tipWindowScore", {
        score: this.windowScorePct,
        pT,
        pH,
      });
    },
...
.sel-panel {
  ...
  &__size-note {
    ...
    &--inverted {
      ...
      .sel-panel__size-note-label {
        color: var(--color-danger);
      }
      .sel-panel__size-note-val {
        color: var(--color-danger);
      }
    }
  }
  ...
  &__ws-label {
    @include mono-upper(0.56rem, 0.07em);
    color: var(--color-text-muted);
    flex-shrink: 0;
  }

...
import CcmHeader from "./components/CcmHeader.vue";
import CcmTipPanel from "./components/CcmTipPanel.vue";
import CcmIdentitySection from "./components/CcmIdentitySection.vue";
import CcmParamsGrid from "./components/CcmParamsGrid.vue";
import CcmResonanceSection from "./components/CcmResonanceSection.vue";
import CcmDerivedPreview from "./components/CcmDerivedPreview.vue";
import CcmFooter from "./components/CcmFooter.vue";
...
const TYPE_DEFAULTS: Record<CellFormType, TypeDefaults> = {
  ...
  bacteria: {
    ...
    density: 1100,
    specificHeatCapacity: 3700,
    resonantFreqGHz: 10,
    capsidQ: 4,
    resonantThresholdVcm: 10000,
  },
  virus: {
    radius: 0.06,
    membraneThickness: 4,
    dielectricConstant: 22,
    conductivity: 0.2,
    thresholdVoltage: 0.4,
    density: 1200,
...
export default defineComponent({
  ...
  computed: {
    ...
    validationErrors(): string[] {
      ...
      if (this.form.membraneThickness <= 0)
        errs.push("Membrane thickness must be > 0.");
      if (this.form.membraneThickness > 200)
        errs.push("Membrane thickness must be ≤ 200 nm.");
      ...
      if (this.form.thresholdVoltage <= 0)
        errs.push("Vm threshold must be > 0.");
      if (this.form.density <= 0) errs.push("Density must be > 0.");
      if (this.form.specificHeatCapacity <= 0)
        errs.push("Specific heat must be > 0.");
      return errs;
    },
  ...
  methods: {
    ...
    onCellTypeChange(ct: CellFormType) {
      ...
      Object.assign(this.form, TYPE_DEFAULTS[ct], {
        cellType: ct,
        label,
        shortLabel,
        notes,
    ...
    onSave() {
      ...
      const preset: Omit<UserCellPreset, "id" | "createdAt"> = {
        ...
        ...(this.form.cellType !== "mammalian" &&
          this.form.resonantFreqGHz != null && {
            resonantFreqGHz: this.form.resonantFreqGHz,
          }),
        ...(this.form.cellType !== "mammalian" &&
          this.form.capsidQ != null && { capsidQ: this.form.capsidQ }),
        ...(this.form.cellType !== "mammalian" &&
          this.form.resonantThresholdVcm != null && {
            resonantThresholdVcm: this.form.resonantThresholdVcm,
          }),
      };

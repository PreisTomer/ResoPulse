...
export default defineComponent({
  ...
  computed: {
    ...
    tipCanvasStr(): string {
      return STATIC_HEATMAP_TOOLTIPS.canvas;
    },

    xMin(): number {
      return HMAP_FREQ_MIN_KHZ;
    },
    xMax(): number {
      return this.cellStore.hmapFreqMaxKHz;
    },
    ...
    yMax(): number {
      ...
      const isPulsed =
        s.waveform === WAVEFORM.PULSED || s.waveform === WAVEFORM.H_FIRE;
      const hfireMult =
        s.waveform === WAVEFORM.H_FIRE ? H_FIRE_THRESHOLD_MULTIPLIER : 1.0;
      const pw_ns = s.pulseWidthNs;

      const hTau = computeTau(s.healthy, sigma);
      const pefH = isPulsed
        ? Math.max(MIN_PULSE_ENVELOPE, 1 - Math.exp((-pw_ns * 1e-9) / hTau))
        : 1.0;
      ...
      const hVthEff =
        tempCorrectedVth(s.healthy.thresholdVoltage, s.healthyTemp) * hfireMult;
      const healthyLysis =
        cosT < 1e-6
          ? 1000
      ...
      if (s.isResonanceMode) {
        ...
        const tResVth =
          ...
          s.target.thresholdVoltage /
            (1.5 * s.target.radius * 1e-4 * Math.max(cosT, 1e-6));
        targetLysis = tempCorrectedVth(tResVth, s.targetTemp) * hfireMult;
      } else {
        const tTau = computeTau(s.target, sigma);
        const pefT = isPulsed
          ? Math.max(MIN_PULSE_ENVELOPE, 1 - Math.exp((-pw_ns * 1e-9) / tTau))
          : 1.0;
        const tVthEff =
          tempCorrectedVth(s.target.thresholdVoltage, s.targetTemp) * hfireMult;
        targetLysis =
          cosT < 1e-6
            ? 1000
    ...
    plotW(): number {
      return this.displayW - HMAP_MARGIN.LEFT - HMAP_MARGIN.RIGHT;
    },
    plotH(): number {
      return this.displayH - HMAP_MARGIN.TOP - HMAP_MARGIN.BOTTOM;
    },
    ...
    physicsKey(): string {
      ...
      return [
        s.healthy.id,
        s.healthy.radius,
        s.healthy.membraneThickness,
        s.healthy.dielectricConstant,
        s.healthy.conductivity,
        s.healthy.thresholdVoltage,
        s.target.id,
        s.target.radius,
        s.target.membraneThickness,
        s.target.dielectricConstant,
        s.target.conductivity,
        s.target.thresholdVoltage,
        t.resonantFreqGHz,
        s.medium,
        s.waveform,
  ...
  methods: {
    ...
    _zoneAt(freqKHz: number, fieldVcm: number): HmapZone {
      ...
      const hTss =
        BODY_TEMP_C +
        (hSAR * dc) / ((NEWTON_COOLING_LAMBDA + hLambdaPerf) * hCp);
      const hVm = computeSchwan(s.healthy, freqKHz, fieldVcm, sigma_e, cosT);
      const hDR =
        (hVm * hPEF) /
        (tempCorrectedVth(s.healthy.thresholdVoltage, hTss) * hfireMult);

      let tDR = 0;
      if (s.isResonanceMode) {
        const tr = s.target as {
        ...
        if (tr.resonantFreqGHz && tr.capsidQ && tr.resonantThresholdVcm) {
          ...
          const effThreshold = tempCorrectedVth(
            tr.resonantThresholdVcm,
            s.targetTemp,
          );
          tDR = computeResonantDisruption(
            tr.resonantFreqGHz,
            tr.capsidQ,
            effThreshold,
            freqKHz * 1000,
        ...
        const tTss =
          BODY_TEMP_C +
          (tSAR * dc) / ((NEWTON_COOLING_LAMBDA + tLambdaPerf) * tCp);
        ...
        tDR =
          (tVm * tPEF) /
          (tempCorrectedVth(s.target.thresholdVoltage, tTss) * hfireMult);
      }
    ...
    _computeGrid() {
      ...
      for (let fi = 0; fi < HMAP_FREQ_STEPS; fi++) {
        ...
        for (let vi = 0; vi < HMAP_FIELD_STEPS; vi++) {
          ...
          if (hTss >= HMAP_THERM_CRIT_C) {
            result[fi * HMAP_FIELD_STEPS + vi] = HMAP_ZONE.THERMAL;
            continue;
          }
          if (hDR >= HMAP_LYSIS_DR) {
            result[fi * HMAP_FIELD_STEPS + vi] = HMAP_ZONE.ABLATIVE;
            continue;
          }
          if (tDR >= HMAP_LYSIS_DR) {
            result[fi * HMAP_FIELD_STEPS + vi] =
              hDR >= HMAP_WARN_DR || hTss >= HMAP_THERM_WARN_C
                ? HMAP_ZONE.MARGINAL
                : HMAP_ZONE.THERAPEUTIC;
            continue;
          }
          result[fi * HMAP_FIELD_STEPS + vi] =
            tDR >= HMAP_APPROACH_DR ? HMAP_ZONE.APPROACHING : HMAP_ZONE.SUB;
        }
      }
    ...
    _renderCanvas() {
      ...
      const ml = HMAP_MARGIN.LEFT,
        mt = HMAP_MARGIN.TOP;
      const pw = this.plotW,
        ph = this.plotH;
      const nF = HMAP_FREQ_STEPS,
        nV = HMAP_FIELD_STEPS;
      ...
      ctx.beginPath();
      ctx.moveTo(opX, mt);
      ctx.lineTo(opX, mt + ph);
      ctx.moveTo(ml, opY);
      ctx.lineTo(ml + pw, opY);
      ctx.stroke();
      ctx.restore();
      ...
      const opZone = this._zoneAt(
        s.currentBroadcastFrequency,
        s.fieldIntensity,
      );
    ...
    _drawAxes(
      ...
      ctx.fillText(
        this.$t("heatmap.axisFreq") as string,
        ml + pw / 2,
        this.displayH,
      );
      ...
      ctx.textBaseline = "middle";
      ctx.fillText(
        `${this.$t("heatmap.axisField") as string} [${UNIT.V_PER_CM}]`,
        0,
        0,
      );
      ctx.restore();
    ...
    _xToFreq(x: number): number {
      ...
      return (
        this.xMin * Math.pow(this.xMax / this.xMin, Math.max(0, Math.min(1, t)))
      );
    },
    ...
    onCanvasMove(e: MouseEvent) {
      ...
      if (
        cx < HMAP_MARGIN.LEFT ||
        cx > rW - HMAP_MARGIN.RIGHT ||
        cy < HMAP_MARGIN.TOP ||
        cy > rH - HMAP_MARGIN.BOTTOM
      ) {
      ...
      // Hover physics
      const s = this.cellStore,
        sigma_e = s.effectiveSigmaE;
      const cosT = s.cosThetaFactor;
      const isHoverPulsed =
        s.waveform === WAVEFORM.PULSED || s.waveform === WAVEFORM.H_FIRE;
      const hoverHfireMult =
        s.waveform === WAVEFORM.H_FIRE ? H_FIRE_THRESHOLD_MULTIPLIER : 1.0;
      const wf = isHoverPulsed ? WF_PULSED : WF_CW;
      const dc = s.dutyCycle,
        pw_ns = s.pulseWidthNs;
      const hCp = s.healthy.specificHeatCapacity;
      const hTau = computeTau(s.healthy, sigma_e);
      const hPEF = isHoverPulsed
        ? Math.max(MIN_PULSE_ENVELOPE, 1 - Math.exp((-pw_ns * 1e-9) / hTau))
        : 1.0;
      const hSAR = computeSAR(s.healthy, fieldVcm, sigma_e, wf);
      const hLPerf = (s.perfusionRate * PENNES_BLOOD_COEFF) / hCp;
      const hTss =
        BODY_TEMP_C + (hSAR * dc) / ((NEWTON_COOLING_LAMBDA + hLPerf) * hCp);
      const hVm = computeSchwan(s.healthy, freqKHz, fieldVcm, sigma_e, cosT);
      const hDR =
        (hVm * hPEF) /
        (tempCorrectedVth(s.healthy.thresholdVoltage, hTss) * hoverHfireMult);
      ...
      if (s.isResonanceMode) {
        ...
        const tTss =
          BODY_TEMP_C + (tSAR * dc) / ((NEWTON_COOLING_LAMBDA + tLPerf) * tCp);
        const tTau = computeTau(s.target, sigma_e);
        const tPEF = isHoverPulsed
          ? Math.max(MIN_PULSE_ENVELOPE, 1 - Math.exp((-pw_ns * 1e-9) / tTau))
          : 1.0;
        const tVm = computeSchwan(s.target, freqKHz, fieldVcm, sigma_e, cosT);
        tDR =
          (tVm * tPEF) /
          (tempCorrectedVth(s.target.thresholdVoltage, tTss) * hoverHfireMult);
      }

      const pLysis =
        !s.isResonanceMode || s.targetCellCategory === CELL_CATEGORY.MAMMALIAN
      ...
      if (
        tDR >= HMAP_LYSIS_DR &&
        hDR < HMAP_LYSIS_DR &&
        hTss < HMAP_THERM_CRIT_C
      ) {
        outcomes.push({
          text: `${ICON.CHECK} ${this.$t("heatmap.outcomeTargetLysis")}`,
          level: "ok",
        });
        if (hDR >= HMAP_WARN_DR || hTss >= HMAP_THERM_WARN_C) {
          outcomes.push({
            text: `${ICON.WARNING} ${this.$t("heatmap.outcomeHealthyStressed")}`,
            level: "warn",
          });
        } else {
          outcomes.push({
            text: `${ICON.CHECK} ${this.$t("heatmap.outcomeHealthySafe")}`,
            level: "ok",
          });
        }
      } else if (hDR >= HMAP_LYSIS_DR) {
        outcomes.push({
          text: `\u26d4 ${this.$t("heatmap.outcomeHealthyLysis")}`,
          level: "danger",
        });
        if (tDR >= HMAP_LYSIS_DR)
          outcomes.push({
            text: `\u26d4 ${this.$t("heatmap.outcomeTargetLysis")}`,
            level: "danger",
          });
      } else if (hTss >= HMAP_THERM_CRIT_C) {
        outcomes.push({
          text: `\u26d4 ${this.$t("heatmap.outcomeThermalDamage", { critC: HMAP_THERM_CRIT_C, unit: UNIT.DEG_C })}`,
          level: "danger",
        });
      } else if (tDR >= HMAP_APPROACH_DR) {
        outcomes.push({
          text: `\u2192 ${this.$t("heatmap.outcomeApproaching")}`,
          level: "info",
        });
        outcomes.push({
          text: `${this.$t("heatmap.outcomeTDrProgress", { pct: (tDR * 100).toFixed(0), lysisAt: (HMAP_LYSIS_DR * 100).toFixed(0) })}`,
          level: "info",
        });
      } else {
        outcomes.push({
          text: `${this.$t("heatmap.outcomeSubThreshold")}`,
          level: "info",
        });
      }
      if (hTss >= HMAP_THERM_WARN_C && hTss < HMAP_THERM_CRIT_C) {
        outcomes.push({
          text: `${ICON.WARNING} ${this.$t("heatmap.outcomeHTempElevated", { temp: hTss.toFixed(1), unit: UNIT.DEG_C })}`,
          level: "warn",
        });
      }
      ...
      const { info, tooltipHtml } = buildHeatmapHoverPresentation({
        freqKHz,
        fieldVcm,
        zone,
        zoneLabel,
      ...
      wrapEl._tipContent = tooltipHtml;
      const brTip = document.getElementById("br-tip");
      if (brTip && brTip.classList.contains("br-tip--on"))
        brTip.innerHTML = tooltipHtml;
    },
    ...
    _resetCanvasTip() {
      ...
      if (wrapEl && wrapEl._tipContent !== undefined) {
        wrapEl._tipContent = STATIC_HEATMAP_TOOLTIPS.canvas;
      }
    },
  },
});

...
<template>
  ...
  <div v-show="open" class="field-panel__accordion-body">
    ...
    >
      <div class="field-panel__row-header">
        <span class="field-panel__row-label" v-tip="STATIC_SLIDER_TOOLTIPS.shellModel">{{ $t('slider.doubleShell') }}</span>
...
import { tipOrientation, tipLysisN, tipLysisNNote, tipDoubleShell, tipPerfusion, tipCellPacking } from '@/tooltips/sliderTooltips'
import {
...
export default defineComponent({
  ...
  computed: {
    ...
    cosThetaDisplay(): string {
      return formatOrientationDisplay(this.cellStore.orientationDeg, this.cellStore.cosThetaFactor)
    },
    ...
    lysisNDisplay(): string {
      return formatLysisNDisplay(this.cellStore.lysisNPulses, this.cellStore.lysisDelayMs)
    },
    ...
    tipLysisN(): string {
      return tipLysisN(buildLysisNTooltipParams({
    ...
    tipDoubleShell(): string {
      return tipDoubleShell(buildDoubleShellTooltipParams({
    ...
    perfusionDisplay(): string {
      return formatPerfusionDisplay(this.cellStore.perfusionRate, this.$t('slider.perfusionInVitro') as string)
    },

    cellPackingDisplay(): string {
      return formatCellPackingDisplay(this.cellStore.cellPackingFraction, this.$t('slider.cellPackingIsolated') as string)
    },
    ...
    tipPerfusionFull(): string {
      const { perfusionRate, effLambdaH } = buildPerfusionTooltipParams({
        ...
        healthySpecificHeatCapacity: this.cellStore.healthy.specificHeatCapacity,
      })

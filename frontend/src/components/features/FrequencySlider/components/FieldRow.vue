...
<template>
  ...
  >
    <div class="field-panel__row-header">
      <span class="field-panel__row-label" v-tip="tipFieldLabel">{{
        $t("slider.fieldIntensity")
      }}</span>
      <div class="field-panel__readout">
        <div v-if="editingField" class="field-panel__readout-edit">
          ...
          />
          <span class="field-panel__readout-input-unit">{{
            fieldInputUnit
          }}</span>
        </div>
        <span
        ...
        <div class="field-panel__badges">
          <span
            ...
            :class="{
              'field-panel__badge--warn':
                targetDisruption > THRESHOLDS.DISRUPTION_WARN,
            }"
        ...
        <div v-if="!editingField" class="field-panel__readout-steps">
          <button
            class="field-panel__readout-step"
            type="button"
            @click="stepField(-1)"
          >
            −
          </button>
          <span class="field-panel__readout-steps-unit">{{
            fieldInputUnit
          }}</span>
          <button
            class="field-panel__readout-step"
...
export default defineComponent({
  ...
  computed: {
    ...
    currentField(): number {
      return this.cellStore.fieldIntensity;
    },
    targetDisruption(): number {
      return this.cellStore.targetDisruptionRatio;
    },
    healthyDisruption(): number {
      return this.cellStore.healthyDisruptionRatio;
    },
    fieldDisplay(): string {
      return formatFieldVcm(this.currentField);
    },
    ...
    fieldInputUnit(): string {
      return this.currentField >= FIELD_KV_THRESHOLD ? "kV/cm" : "V/cm";
    },
    fieldInputStep(): number {
      return this.currentField >= FIELD_KV_THRESHOLD ? 0.1 : 1;
    },
    ...
    fieldMarkers(): Array<{ id: string; pct: number; color: string }> {
      ...
      return [
        {
          id: "lysis-t",
          pct: linPct(this.cellStore.targetLysisField),
          color: "danger",
        },
        {
          id: "lysis-h",
          pct: linPct(this.cellStore.healthyLysisField),
          color: "healthy",
        },
    ...
    tipTargetBadgeLabel(): string {
      return tipTargetBadge({
        ...buildTargetBadgeTooltipParams({
          ...
          targetDisruption: this.targetDisruption,
          targetVm: this.cellStore.targetVm,
    ...
    tipHealthyBadgeLabel(): string {
      return tipHealthyBadge({
        ...buildHealthyBadgeTooltipParams({
          ...
          healthyVm: this.cellStore.healthyVm,
        }),
        t: this.$t.bind(this),
      });
    },
  ...
  methods: {
    onFieldInput(e: Event) {
      this.cellStore.setFieldIntensity(
    ...
    startEditField() {
      this.fieldInputVal =
        this.currentField >= FIELD_KV_THRESHOLD
    ...
    commitFieldEdit() {
      ...
      const clamped = Math.max(
        this.sliderRanges.fieldMin,
        Math.min(this.sliderRanges.fieldMax, vcm),
      );
    ...
    stepField(dir: number) {
      const stepVcm =
        this.fieldInputUnit === "kV/cm"
          ? this.fieldInputStep * 1_000
          : this.fieldInputStep;
      const next = Math.max(
        this.sliderRanges.fieldMin,
        Math.min(this.sliderRanges.fieldMax, this.currentField + dir * stepVcm),
      );
    ...
    onScrubFieldMove(e: MouseEvent) {
      ...
      const clamped = Math.max(
        this.sliderRanges.fieldMin,
        Math.min(this.sliderRanges.fieldMax, newField),
      );
      this.cellStore.setFieldIntensity(clamped);
      broadcastStateSync();
    },

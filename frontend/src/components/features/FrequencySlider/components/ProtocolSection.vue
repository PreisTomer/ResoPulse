    ...
    <div v-show="open" class="field-panel__accordion-body">
      ...
      >
        <div class="field-panel__row-header">
          <span class="field-panel__row-label">{{
            $t("slider.waveform")
          }}</span>
            ...
            <label
              ...
              :class="{
                'field-panel__pill--active':
                  currentWaveform === WAVEFORM.PULSED,
              }"
            >
              <input
                type="radio"
                :value="WAVEFORM.PULSED"
                :checked="currentWaveform === WAVEFORM.PULSED"
                name="waveform"
                @change="onWaveformChange(WAVEFORM.PULSED)"
              />
              {{ $t("slider.pulsed") }}
            </label>
            <label
              class="field-panel__pill"
              :class="{
                'field-panel__pill--active': currentWaveform === WAVEFORM.CW,
              }"
            >
              <input
                type="radio"
                :value="WAVEFORM.CW"
                :checked="currentWaveform === WAVEFORM.CW"
                name="waveform"
                @change="onWaveformChange(WAVEFORM.CW)"
              />
              {{ $t("slider.cw") }}
            </label>
            <label
              ...
              :class="{
                'field-panel__pill--active':
                  currentWaveform === WAVEFORM.H_FIRE,
              }"
              v-tip="$t('slider.tipHfire')"
            >
              <input
                type="radio"
                :value="WAVEFORM.H_FIRE"
                :checked="currentWaveform === WAVEFORM.H_FIRE"
                name="waveform"
                @change="onWaveformChange(WAVEFORM.H_FIRE)"
              />
              {{ $t("slider.hfire") }}
            </label>
          </div>
      ...
      <div
        ...
        :class="
          thermalDangerLevel !== THERMAL_LEVEL.SAFE
            ? `field-panel__row--${thermalDangerLevel}`
            : ''
        "
      >
        <div class="field-panel__row-header">
          ...
          <div class="field-panel__readout">
            <span
              ...
              :class="
                thermalDangerLevel !== THERMAL_LEVEL.SAFE
                  ? `field-panel__readout--${thermalDangerLevel}`
                  : ''
              "
              v-tip="tipDutyCycle"
            ...
            <span class="field-panel__readout-sub" v-tip="tipDutyCycle"
              >T_ss {{ maxSteadyTemp.toFixed(0) }} {{ UNIT.DEG_C }}</span
            >
            <span class="field-panel__readout-sub" v-tip="tipDutyCycle"
              >SAR_eff {{ CELL_LABEL.TARGET }}
              {{ (cellStore.targetSAR * cellStore.dutyCycle).toFixed(1) }}
              {{ UNIT.W_PER_KG }}</span
            >
          </div>
        </div>
      ...
      <div
        v-if="isTimedWaveform"
        class="field-panel__row field-panel__row--compact-readout field-panel__row--pw"
      >
        <div class="field-panel__row-header">
          <span class="field-panel__row-label" v-tip="tipPulseWidth">{{
            $t("slider.pulseWidth")
          }}</span>
          <div class="field-panel__readout">
            <span class="field-panel__readout-value" v-tip="tipPulseWidth">{{
              pulseWidthDisplay
            }}</span>
            <span class="field-panel__readout-sub" v-tip="tipPulseWidth"
              >Lysis {{ lysisTimeDisplay }}</span
            >
            <span
              class="field-panel__readout-sub"
              :class="`field-panel__readout-sub--pw-${pwSufficiency}`"
              v-tip="tipMinPulse"
              >{{ minPwDisplay }}</span
            >
          </div>
...
export default defineComponent({
  ...
  props: {
    ...
    thermalDangerLevel: {
      type: String as PropType<
        "safe" | "hyperthermic" | "denaturing" | "vaporizing"
      >,
      required: true,
    },
  ...
  computed: {
    ...
    currentWaveform(): "cw" | "pulsed" | "hfire" {
      return this.cellStore.waveform;
    },

    isTimedWaveform(): boolean {
      return (
        this.currentWaveform === WAVEFORM.PULSED ||
        this.currentWaveform === WAVEFORM.H_FIRE
      );
    },

    waveformMetaLabel(): string {
      if (this.currentWaveform === WAVEFORM.CW)
        return `wf×${CW_WAVEFORM_FACTOR}`;
      if (this.currentWaveform === WAVEFORM.H_FIRE)
        return `wf×${PULSED_WAVEFORM_FACTOR} thr×${H_FIRE_THRESHOLD_MULTIPLIER}`;
      return `wf×${PULSED_WAVEFORM_FACTOR}`;
    },

    dutyCycleLogVal(): number {
      return Math.log10(this.cellStore.dutyCycle);
    },

    dutyCycleDisplay(): string {
      return formatDutyCycleDisplay(this.cellStore.dutyCycle);
    },

    pulseWidthLogVal(): number {
      return Math.log10(this.cellStore.pulseWidthNs);
    },

    pulseWidthDisplay(): string {
      return formatPulseWidthDisplay(this.cellStore.pulseWidthNs);
    },
    ...
    lysisTimeDisplay(): string {
      return buildLysisTimeDisplay(this.cellStore.lysisDelayMs);
    },

    tauTargetNs(): number {
      return this.cellStore.targetFc > 0
        ? 1e6 / (2 * Math.PI * this.cellStore.targetFc)
        : 0;
    },

    minPwNs(): number {
      return 3 * this.tauTargetNs;
    },

    minPwDisplay(): string {
      return formatMinPulseDisplay(this.minPwNs);
    },
    ...
    tipMinPulse(): string {
      return this.$t("slider.tipMinPulse");
    },
  },
...
.field-panel {
  ...
  &__row {
    ...
    &--hyperthermic .field-panel__slider {
      &::-webkit-slider-thumb {
        ...
        box-shadow: 0 0 6px
          color-mix(in srgb, var(--color-amber) 50%, transparent);
      }
      &::-moz-range-thumb {
        background: var(--color-amber);
      }
    ...
    &--denaturing .field-panel__slider {
      &::-webkit-slider-thumb {
        ...
        box-shadow: 0 0 8px
          color-mix(in srgb, var(--color-orange) 70%, transparent);
      }
      &::-moz-range-thumb {
        background: var(--color-orange);
      }
    ...
    &--vaporizing .field-panel__slider {
      &::-webkit-slider-thumb {
        ...
        box-shadow: 0 0 10px
          color-mix(in srgb, var(--color-danger) 90%, transparent);
  ...
  &__pill {
    ...
    &--hfire {
      ...
      &.field-panel__pill--active {
        ...
        box-shadow: 0 0 8px
          color-mix(in srgb, var(--color-amber) 28%, transparent);
      }
    }
    &--nuclear {
      border-color: color-mix(
  ...
  &__readout {
    ...
    &--vaporizing {
      ...
      animation: state-blink 0.5s ease-in-out infinite;
    }
  }
}
</style>

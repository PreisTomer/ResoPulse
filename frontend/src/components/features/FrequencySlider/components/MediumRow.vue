...
<template>
  <div id="hl-medium-row" class="field-panel__row field-panel__row--medium">
    <span class="field-panel__row-label" v-tip="tipMediumLabel">{{
      $t("slider.medium")
    }}</span>
    <select
      class="field-panel__medium-select"
      :value="currentMedium"
      v-tip="STATIC_SLIDER_TOOLTIPS.mediumKeys[currentMedium]"
      @change="
        onMediumChange(
          ($event.target as HTMLSelectElement).value as typeof currentMedium,
        )
      "
    >
    ...
    <span class="field-panel__row-meta" v-tip="tipSigmaELabel"
...
import { tipMedium, tipSigmaE } from "@/tooltips/sliderTooltips";
...
export default defineComponent({
  data() {
    return { MEDIA, STATIC_SLIDER_TOOLTIPS };
  },
  ...
  computed: {
    ...
    currentMedium(): MediumKey {
      return this.cellStore.medium;
    },
    tipMediumLabel(): string {
      return tipMedium(this.currentMedium);
    },
    tipSigmaELabel(): string {
      return tipSigmaE(this.MEDIA[this.currentMedium].conductivity);
    },
  },

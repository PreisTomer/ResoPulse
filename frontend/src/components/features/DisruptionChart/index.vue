...
<template>
  <div class="dr-chart">
    <div class="dr-chart__header">
      <span class="dr-chart__title" v-tip="$t('drChart.tipTitle')">{{
        $t("drChart.title")
      }}</span>
      <div class="dr-chart__header-right">
        <button
          ...
          @click="toggleZoom"
        >
          {{ zoomMode ? $t("drChart.zoomFull") : $t("drChart.zoomIn") }}
        </button>
        <DrChartLegend />
      </div>
    </div>
    <div ref="chartEl" class="dr-chart__svg-wrap">
      ...
      <div v-if="showDisclaimer" class="dr-chart__disclaimer">
        ...
        <span class="dr-chart__disclaimer-text">{{
          isDisclaimerBacteriaVirus
            ? $t("drChart.disclaimerBacteriaVirus")
            : $t("drChart.disclaimerLowField")
        }}</span>
      </div>
    </div>
  </div>
...
import DrChartLegend from "./components/DrChartLegend.vue";
import DrChartTooltip from "./components/DrChartTooltip.vue";
import type { HoverInfo } from "./components/DrChartTooltip.vue";
import {
  F_MIN_HZ,
  F_MAX_HZ,
  Y_MIN_MAX,
...
export default defineComponent({
  ...
  watch: {
    ...
    "cellStore.pulseEnvelopeFactorHealthy": {
      handler() {
        this.updateChart();
      },
    },
    "cellStore.pulseEnvelopeFactorTarget": {
      handler() {
        this.updateChart();
      },
    },
  ...
  methods: {
    ...
    initChart() {
      ...
      clearChartContainer(container, "svg");

      const svgEl = createSvgRoot(container, totalW, totalH).style(
        "overflow",
        "hidden",
      );
    ...
    updateChart() {
      ...
      // ── Compute curves ────────────────────────────────────────────────────
      const isAcousticTarget =
        this.cellStore.targetCellCategory === CELL_CATEGORY.BACTERIA ||
      ...
      const tNominalVth =
        isAcousticTarget && tRes.resonantThresholdVcm
      ...
      gc.select<SVGLineElement>(".thresh-rev-ep")
        .attr("x1", 0)
        .attr("x2", W)
        .attr("y1", yS(DR_REV_EP))
        .attr("y2", yS(DR_REV_EP))
    ...
    updateCursor() {
      ...
      g.select<SVGTextElement>(".cursor-drag-hint").attr("x", cx);
    },
    ...
    onHover(event: MouseEvent) {
      ...
      const idx = Math.max(
        0,
        Math.min(this._curveData.length - 1, bisect(this._curveData, mouseHz)),
      );

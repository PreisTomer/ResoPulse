...
<template>
  <div class="pop-controls">
    <div class="pop-controls__ctrl-group">
      <span
        class="pop-controls__ctrl-label"
        v-tip="$t('population.tipPopSize')"
    ...
    <div class="pop-controls__ctrl-group">
      ...
      <input
        ...
        "
      />
      <span class="pop-controls__ctrl-unit">{{
        $t("population.varianceUnit")
      }}</span>
    </div>
    ...
    <div class="pop-controls__ctrl-group">
      <span
        ...
        >{{ $t("population.ctrlVThVarianceLabel") }}</span
      >
      <input
        class="pop-controls__input"
        type="number"
    ...
    >
      {{ $t("population.resampleBtn") }}
    </button>

    <!-- Histogram Y-axis mode toggle -->
    <div
      class="pop-controls__norm-toggle"
      v-tip="$t('population.tipNormalize')"
    >
      <button
        class="pop-controls__norm-btn"
        :class="{ 'pop-controls__norm-btn--active': !normalizeChart }"
        @click="
          $emit(EMIT.UPDATE_NORMALIZE_CHART, false);
          $emit(EMIT.REDRAW);
        "
      >
        {{ $t("population.normCount") }}
      </button>
      <button
        class="pop-controls__norm-btn"
        :class="{ 'pop-controls__norm-btn--active': normalizeChart }"
        @click="
          $emit(EMIT.UPDATE_NORMALIZE_CHART, true);
          $emit(EMIT.REDRAW);
        "
      >
        {{ $t("population.normFraction") }}
      </button>
    </div>

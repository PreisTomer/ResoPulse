...
<template>
  <div class="experiment__snap-bar" v-tip="tipSnapBar">
    ...
    <span class="experiment__snap-bar-range">
      ...
      {{
        sweepWindow.param === "field"
          ? $t("sweep.fieldUnit")
          : $t("sweep.freqUnit")
      }}
    </span>
    <span class="experiment__snap-bar-affects"
      >{{
        sweepWindow.param === "field"
          ? $t("exp.snapBarSubField")
          : $t("exp.snapBarSubFreq")
      }}
      {{ Math.round((sweepWindow.lo + sweepWindow.hi) / 2) }}
      {{
        sweepWindow.param === "field"
          ? $t("sweep.fieldUnit")
          : $t("sweep.freqUnit")
    ...
    >
    <span v-if="snapConfirming" class="experiment__snap-bar-lysis-warn">{{
      $t("exp.snapBarLysisWarn", { cellLabel: snapLysisCellLabel })
    }}</span>
    <div class="experiment__snap-confirm-row">
      ...
      <template v-else-if="snapConfirming">
        <button
          class="experiment__snap-bar-btn experiment__snap-bar-btn--confirm"
          @click="snapToWindow"
        >
          {{ $t("exp.snapBarBtnConfirm") }}
        </button>
        <button
          class="experiment__snap-bar-btn experiment__snap-bar-btn--cancel"
          @click="cancelSnap"
        >
      ...
      <template v-else>
        <button
          class="experiment__snap-bar-btn experiment__snap-bar-btn--confirmed"
          disabled
        >
...
import { formatSnapLysisCellLabel } from "../lib";
...
export default defineComponent({
  ...
  computed: {
    ...
    snapLysisCellLabel(): string {
      return formatSnapLysisCellLabel(
    ...
    isSnapIdle(): boolean {
      return !this.snapConfirming && !this.snapConfirmed;
    },
  },
  ...
  methods: {
    snapToWindow() {
      ...
      const center = Math.round(
        (this.sweepWindow.lo + this.sweepWindow.hi) / 2,
      );
...
.experiment__snap-bar {
  ...
  background: linear-gradient(
    90deg,
    color-mix(in srgb, var(--color-ok) 8%, transparent) 0%,
    color-mix(in srgb, var(--color-ok) 4%, transparent) 100%
  );

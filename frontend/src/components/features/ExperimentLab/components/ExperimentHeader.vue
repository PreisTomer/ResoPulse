  ...
  <div id="hl-setup-bar" class="experiment__header">
    ...
    <div id="hl-cell-badges" class="experiment__cell-badges">
      ...
      <div class="experiment__cell-slot">
        ...
        >
          <div
            class="experiment__cell-badge-row"
            :class="{ 'experiment__cell-badge-row--open': healthyPickerOpen }"
          >
            <span class="experiment__cell-badge-type">{{
              $t("exp.badgeHealthy")
            }}</span>
            <span
              class="experiment__cell-badge-selected experiment__cell-badge-selected--healthy"
              >{{ healthyLabelShort }}</span
            >
            <span
              class="experiment__cell-badge-caret"
              :class="{
                'experiment__cell-badge-caret--open': healthyPickerOpen,
              }"
              >{{ ICON.EXPAND }}</span
            >
          </div>
        ...
        <div v-if="healthyPickerOpen" class="experiment__cell-picker">
          <div class="experiment__cell-picker-title">
            {{ $t("exp.pickerHealthyTitle") }}
          </div>
          <div class="experiment__cell-picker-grid">
            <button
              ...
              :class="{
                'experiment__preset-btn--active':
                  cellStore.healthy.id === p.presetId,
              }"
              @click="loadHealthyPreset(p)"
            >
              <span class="experiment__preset-btn-name">{{
                p.shortLabel
              }}</span>
      ...
      <div class="experiment__cell-slot">
        ...
        >
          <div
            class="experiment__cell-badge-row"
            :class="{ 'experiment__cell-badge-row--open': targetPickerOpen }"
          >
            <span class="experiment__cell-badge-type">{{
              $t("exp.badgeTarget")
            }}</span>
            <span
              class="experiment__cell-badge-selected experiment__cell-badge-selected--target"
              >{{ cellStore.target.label }}</span
            >
            <span
              class="experiment__cell-badge-caret"
              :class="{
                'experiment__cell-badge-caret--open': targetPickerOpen,
              }"
              >{{ ICON.EXPAND }}</span
            >
          </div>
        ...
        <div v-if="targetPickerOpen" class="experiment__cell-picker">
          <div class="experiment__cell-picker-hdr">
            ...
            <div class="experiment__cell-picker-tabs">
              <button
                ...
                :class="{
                  'experiment__cell-picker-tab--active':
                    targetPickerCategory === cat,
                }"
                :style="
                  targetPickerCategory === cat
                    ? {
                        borderColor: GROUP_COLORS[cat],
              ...
              <button
                ...
                :class="{
                  'experiment__cell-picker-tab--active':
                    targetPickerCategory === 'custom',
                }"
                @click.stop="targetPickerCategory = 'custom'"
              >
                {{ $t("userPresets.tabLabel") }}
              </button>
            </div>
          </div>
          <!-- Built-in presets grid -->
          <div
            v-if="targetPickerCategory !== 'custom'"
            class="experiment__cell-picker-grid"
          >
            <button
              ...
              :class="{
                'experiment__preset-btn--active':
                  cellStore.target.id === p.presetId,
              }"
              :style="
                cellStore.target.id === p.presetId
                  ? {
                      borderColor:
                        GROUP_COLORS[targetPickerCategory as CellGroup],
                      color: GROUP_COLORS[targetPickerCategory as CellGroup],
                    }
                  : {}
              "
          ...
          <div v-else class="experiment__cell-picker-grid">
            <p
              v-if="!userPresetsStore.hasPresets"
              class="experiment__custom-empty"
            >
              {{ $t("userPresets.emptyMsg") }}<br />
              <span class="experiment__custom-hint">{{
                $t("userPresets.emptyHint")
              }}</span>
            </p>
            <div
              ...
              :class="{
                'experiment__preset-btn--active': cellStore.target.id === p.id,
              }"
              role="button"
              tabindex="0"
            ...
            >
              ...
              <span class="experiment__preset-btn-sub">{{
                p.notes || p.label
              }}</span>
              <button
                class="experiment__preset-btn-del"
                @click.stop="userPresetsStore.remove(p.id)"
                :title="$t('exp.deletePreset')"
              >
                {{ ICON.CLOSE }}
              </button>
            </div>
            <button
              class="experiment__preset-btn-new"
    ...
    <div class="experiment__header-right">
      ...
      >
        ...
        {{ $t("exp.zDriftLabel") }}
        {{ impedanceStore.impedanceDriftPct.toFixed(1) }}%
      </RouterLink>
      <span
        class="experiment__chip"
        :class="
          socketConnected
            ? 'experiment__chip--connected'
            : 'experiment__chip--local'
        "
        v-tip="socketConnected ? $t('exp.connectedTip') : $t('exp.localTip')"
      >
        <span
          class="experiment__chip-dot"
          :class="socketConnected ? '' : 'experiment__chip-dot--warn'"
        ></span>
        {{
          socketConnected
            ? $t("exp.connected").toUpperCase()
            : $t("exp.local").toUpperCase()
        }}
      </span>
    </div>
  </div>
...
import { CreateCellModal } from "@/components/features";

import {
  tipCellBadgeHealthy,
  tipCellBadgeTarget,
} from "@/tooltips/experimentTooltips";
import { buildExperimentCellBadgeTooltip } from "../lib";
...
export default defineComponent({
  ...
  computed: {
    ...
    tipHealthyBadge(): string {
      return buildExperimentCellBadgeTooltip(
        tipCellBadgeHealthy,
...
.experiment {
    ...
    &-tab {
      ...
      &--custom {
        ...
        &.experiment__cell-picker-tab--active {
          border-color: color-mix(
            in srgb,
            var(--color-vibrating) 70%,
            transparent
          );
          color: var(--color-vibrating);
          background: color-mix(
            in srgb,
            var(--color-vibrating) 8%,
            transparent
          );
        }
      }
  ...
  &__preset-btn {
    ...
    &--healthy.experiment__preset-btn--active {
      border-color: var(--color-primary);
    }
    ...
    &-new {
      ...
      border: 1px dashed
        color-mix(in srgb, var(--color-vibrating) 35%, transparent);
      border-radius: 4px;

    ...
    <div class="exp-log__table-wrap">
      <table class="exp-log__table">
        ...
        <tbody>
          ...
          >
            <td v-if="showSessionCol" class="exp-log__td-session" v-tip="logTooltips.session(e)">{{ e.sessionName ?? NULL_DISPLAY }}</td>
            <td class="exp-log__td-id">{{ e.id }}</td>
            <td class="exp-log__td-mono">{{ e.timestamp }}</td>
            <td class="exp-log__td-mono" v-tip="logTooltips.freq(e)">{{ formatFreqKHz(e.freqKHz) }}</td>
            <td class="exp-log__td-mono" v-tip="logTooltips.field(e)">{{ e.fieldVcm != null ? e.fieldVcm : NULL_DISPLAY }}</td>
            <!-- Schwan mode columns -->
            <td v-if="!isResonanceMode" class="exp-log__td-target" v-tip="logTooltips.targetVm(e)">{{ e.targetVm }}</td>
            <td v-if="!isResonanceMode" class="exp-log__td-healthy" v-tip="logTooltips.healthyVm(e)">{{ e.healthyVm }}</td>
            <td v-if="!isResonanceMode" class="exp-log__td-sel" v-tip="logTooltips.selectivity(e)">{{ e.selectivity.toFixed(2) }}</td>
            <td v-if="!isResonanceMode" class="exp-log__td-dep" :class="depKClass(e.depHealthyK)" v-tip="logTooltips.depHealthy(e)">{{ depKDisplay(e.depHealthyK) }}</td>
            <td v-if="!isResonanceMode" class="exp-log__td-dep" :class="depKClass(e.depTargetK)" v-tip="logTooltips.depTarget(e)">{{ depKDisplay(e.depTargetK) }}</td>
            ...
            <td v-if="isResonanceMode" class="exp-log__td-target" v-tip="logTooltips.targetRatio(e)">{{ formatOptionalRatioPct(e.targetRatio) }}</td>
            <td v-if="isResonanceMode" class="exp-log__td-healthy-dr" v-tip="logTooltips.healthyRatio(e)">{{ formatOptionalRatioPct(e.healthyRatio) }}</td>
            <td class="exp-log__td-event">
              <StatusBadge :label="e.event" :variant="eventVariant(e.event)" :tooltip="logTooltips.event(e)" />
            </td>
...
import { createExperimentLogTooltips, formatDoseBadge, formatOptionalRatioPct } from '../lib'
...
export default defineComponent({
  ...
  methods: {
    ...
    formatOptionalRatioPct(ratio?: number): string { return formatOptionalRatioPct(ratio) },

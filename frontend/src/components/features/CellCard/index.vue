...
import { CellBody, CellHeader, CellParamsPanel, CellVisual } from './components'
import {
  buildStateTooltip,
  buildTempDisplay,
  buildTempTooltip,
  ...
  computed: {
    ...
    editableParams() {
      ...
      return paramSet.map((p) => ({
        ...p,
        displayValue: (this.currentCell as object as Record<string, number>)[p.key] ?? 0,
      }))
    },
    ...
    derivedParams() {
      ...
      if (this.isAcousticTarget) {
        const acousticCell = this.currentCell as CellConfig & { resonantFreqGHz?: number; capsidQ?: number }

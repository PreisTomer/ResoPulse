...
describe("cellCardModel helpers", () => {
  ...
  it("uses waveform-aware effective thresholds for the selected cell", () => {
    ...
    ).toBeCloseTo(2 * H_FIRE_THRESHOLD_MULTIPLIER, 6);
  });

  it("detects acoustic targets only for resonant bacteria or virus presets", () => {
    expect(
      isAcousticTargetCell("healthy", "bacteria", {
        resonantFreqGHz: 1,
      } as typeof targetCell),
    ).toBe(false);
    expect(
      isAcousticTargetCell("target", "mammalian", {
        resonantFreqGHz: 1,
      } as typeof targetCell),
    ).toBe(false);
    expect(
      isAcousticTargetCell("target", "virus", {
        resonantFreqGHz: 1,
      } as typeof targetCell),
    ).toBe(true);
  });
  ...
  it("builds shared Vm and temperature display strings and flags", () => {
    ...
    expect(getTempFlags(41)).toEqual({
      tempWarning: false,
      tempDenaturing: false,
      tempVaporizing: false,
    });
    expect(getTempFlags(65)).toEqual({
      tempWarning: true,
      tempDenaturing: true,
      tempVaporizing: false,
    });
  ...
  it("builds shared Vm, temperature, and state tooltips", () => {
    ...
    expect(buildTempTooltip("42.0 °C", getTempFlags(42))).toContain(
      "Cell Temperature",
    );
    expect(buildStateTooltip("stable", false, "healthy", 2500)).toContain(
      "Cell State",
    );
  });
});

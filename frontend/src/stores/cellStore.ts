...
function lysisField(
  ...
  return (
    (cell.thresholdVoltage * hfireMult * Math.sqrt(1 + (omega * tau) ** 2)) /
    (SCHWAN_SPHERE_FACTOR *
      cell.radius *
      1e-6 *
      cosTheta *
      100 *
      Math.max(MIN_PULSE_ENVELOPE, pef))
  );
}
...
export const useCellStore = defineStore("cell", {
  ...
  getters: {
    ...
    cosThetaFactor: (state): number =>
      Math.abs(Math.cos((state.orientationDeg * Math.PI) / 180)),
    isResonanceMode: (state): boolean =>
    ...
    lysisDelayMs: (state): number => {
      if (
        (state.waveform !== WAVEFORM.PULSED &&
          state.waveform !== WAVEFORM.H_FIRE) ||
        state.dutyCycle >= 1
      )
        return LYSIS_DELAY_CW_MS;
      const pulsePeriodMs = (state.pulseWidthNs * NS_TO_MS) / state.dutyCycle;
      return Math.max(
        LYSIS_DELAY_CW_MS,
        Math.min(LYSIS_DELAY_MAX_MS, state.lysisNPulses * pulsePeriodMs),
      );
    },

    effectiveDutyCycle: (state): number =>
      state.waveform === WAVEFORM.CW ? 1.0 : state.dutyCycle,

    // virus: R<0.1µm, bacteria: R<2µm, mammalian: R>=2µm
    targetCellCategory: (state): "mammalian" | "bacteria" | "virus" => {
      if (state.target.radius < THRESHOLDS.RADIUS_VIRUS_MAX)
        return CELL_CATEGORY.VIRUS;
      if (state.target.radius < THRESHOLDS.RADIUS_BACTERIA_MAX)
        return CELL_CATEGORY.BACTERIA;
      return CELL_CATEGORY.MAMMALIAN;
    },

    systemReady: (state): boolean =>
      state.healthyTemp < THRESHOLDS.TEMP_WARN &&
      state.targetTemp < THRESHOLDS.TEMP_WARN,
    ...
    depForceScale: (state): number =>
      state.waveform === WAVEFORM.CW ? 0.5 : state.dutyCycle,
    ...
    healthyVm(): number {
      ...
      return computeSchwan(
        state.healthy,
        state.currentBroadcastFrequency,
    ...
    healthyDisruptionRatio(): number {
      const state = this as CellStoreState;
      const vthEff = effectiveElectroporationThreshold(
        state.healthy.thresholdVoltage,
        state.healthyTemp,
        state.waveform,
      );
      return (this.healthyVm * this.pulseEnvelopeFactorHealthy) / vthEff;
    },
    ...
    targetDisruptionRatio(): number {
      ...
      if (
        ...
        const effThreshold = effectiveResonanceThreshold(
      ...
      const vthEff = effectiveElectroporationThreshold(
        state.target.thresholdVoltage,
        state.targetTemp,
        state.waveform,
      );
      return (this.targetVm * this.pulseEnvelopeFactorTarget) / vthEff;
    },
    ...
    therapeuticIndex(): number {
      return safeRatio(
        this.targetDisruptionRatio,
        this.healthyDisruptionRatio,
        THRESHOLDS.TI_DISPLAY_CAP,
        NEAR_ZERO_DR,
      );
    },
    ...
    tiUncertaintyRange(): { low: number; high: number } {
      ...
      if (this.isResonanceMode) {
        ...
        if (
          isActiveResonanceTarget(t, cat, this.isResonanceMode) &&
          t.capsidQMin !== undefined &&
          t.capsidQMax !== undefined
        ) {
          // Acoustic resonance threshold: temperature correction only — hfireMult does not apply
          const effThr = effectiveResonanceThreshold(
            t.resonantThresholdVcm,
            state.targetTemp,
          );
          ...
          const drTMin = computeResonantDisruption(
            t.resonantFreqGHz,
            t.capsidQMin,
            effThr,
            freqHz,
          ...
          const drTMax = computeResonantDisruption(
            t.resonantFreqGHz,
            t.capsidQMax,
            effThr,
            freqHz,
      ...
      const vmTLow = computeSchwan(
        {
          ...state.target,
          conductivity: state.target.conductivity * (1 - uncT),
        },
        freq,
        field,
        sigma_e,
      ...
      const vmHHigh = computeSchwan(
        {
          ...state.healthy,
          conductivity: state.healthy.conductivity * (1 + uncH),
        },
        freq,
        field,
        sigma_e,
      ...
      const vthT = effectiveElectroporationThreshold(
        state.target.thresholdVoltage,
        state.targetTemp,
        state.waveform,
      );
      const vthH = effectiveElectroporationThreshold(
        state.healthy.thresholdVoltage,
        state.healthyTemp,
        state.waveform,
      );
      const drTLow = (vmTLow * pefT) / vthT;
      const drHHigh = (vmHHigh * pefH) / vthH;
      const tiLow = Math.max(
        0,
        safeRatio(drTLow, drHHigh, THRESHOLDS.TI_DISPLAY_CAP, NEAR_ZERO_DR),
      );
      // TI_high: strongest target + weakest healthy coupling
      const vmTHigh = computeSchwan(
        {
          ...state.target,
          conductivity: state.target.conductivity * (1 + uncT),
        },
      ...
      const drTHigh = (vmTHigh * pefT) / vthT;
      const drHLow = (vmHLow * pefH) / vthH;
      const tiHigh = safeRatio(
        drTHigh,
        drHLow,
        THRESHOLDS.TI_DISPLAY_CAP,
        NEAR_ZERO_DR,
    ...
    healthyNuclearDisruptionRatio(): number {
      const state = this as CellStoreState;
      const vth = effectiveElectroporationThreshold(
        state.healthy.nuclearThresholdVoltage ?? THRESHOLDS.NUCLEAR_VM_DEFAULT,
        state.healthyTemp,
        state.waveform,
      );
      ...
      return (this.healthyNuclearVm * this.pulseEnvelopeFactorHealthy) / vth;
    },

    targetNuclearDisruptionRatio(): number {
      const state = this as CellStoreState;
      const vth = effectiveElectroporationThreshold(
        state.target.nuclearThresholdVoltage ?? THRESHOLDS.NUCLEAR_VM_DEFAULT,
        state.targetTemp,
        state.waveform,
      );
      // Nuclear membrane is gated by outer membrane charging: apply outer PEF so nuclear DR
      // correctly approaches 0 for short pulses (t_p << τ_out), consistent with outer DR.
      return (this.targetNuclearVm * this.pulseEnvelopeFactorTarget) / vth;
    },

    nuclearSelectivityRatio(): number {
      return safeRatio(
        this.targetNuclearDisruptionRatio,
        this.healthyNuclearDisruptionRatio,
        THRESHOLDS.TI_DISPLAY_CAP,
        NEAR_ZERO_DR,
      );
    },
    ...
    targetLysisField(): number {
      const state = this as CellStoreState;
      const hfireMult = getHfireThresholdMultiplier(state.waveform);
      return lysisField(
        state.target,
        state.currentBroadcastFrequency,
        this.effectiveSigmaE,
        this.cosThetaFactor,
        this.pulseEnvelopeFactorTarget,
        hfireMult,
      );
    },
    ...
    healthyLysisField(): number {
      ...
      return lysisField(
        ...
        this.effectiveSigmaE,
        this.cosThetaFactor,
        this.pulseEnvelopeFactorHealthy,
        hfireMult,
      );
    },
    ...
    healthySteadyStateTemp(): number {
      ...
      return Math.min(
        BODY_TEMP_C + sar_eff / ((NEWTON_COOLING_LAMBDA + lambda_perf) * cp),
        THRESHOLDS.TEMP_CAP,
      );
    },
    ...
    bulkMediumSteadyStateTempC(): number {
      ...
      const lambda_perf =
        ((this as CellStoreState).perfusionRate * PENNES_BLOOD_COEFF) / cp_m;
      return Math.min(
        BODY_TEMP_C +
          (this.mediumJouleHeatingSAR * dc) /
            ((NEWTON_COOLING_LAMBDA + lambda_perf) * cp_m),
        THRESHOLDS.TEMP_CAP,
      );
    },
    ...
    pulsedEnergyDensity_mJcm3(): number {
      ...
      const wf = state.waveform === WAVEFORM.CW ? WF_CW : WF_PULSED;
      const tp_s =
        state.waveform === WAVEFORM.CW ? 1.0 : state.pulseWidthNs * NS_TO_S;
    ...
    depHealthySecondCrossoverKHz(): number {
      const state = this as CellStoreState;
      const eps_r = MEDIA[state.medium].permittivity;
      return computeDepSecondCrossoverKHz(
        state.healthy,
        this.effectiveSigmaE,
        eps_r,
      );
    ...
    targetResealingTimeS(): number {
      ...
      if (
        dr < THRESHOLDS.HEALTHY_APPROACHING ||
        dr >= THRESHOLDS.DISRUPTION_WARN
      )
        return 0;
      const tempC = state.targetTemp;
      const nPulses =
        state.waveform === WAVEFORM.CW ? 1 : Math.max(1, state.lysisNPulses);
      const drTerm = Math.pow(
        Math.min(1, dr) / RESEAL_DR_REF,
        RESEAL_DR_EXPONENT,
      );
      const tempTerm = Math.exp(
        -RESEAL_TEMP_COEFF * Math.max(0, tempC - BODY_TEMP_C),
      );
    ...
    healthyStimIndex(): number {
      const r = Math.min(
        1,
        this.healthyDisruptionRatio / THRESHOLDS.NOURISHING,
      );
    ...
    healthyMildThermalActivation(): number {
      ...
      if (T <= THERMAL_MA_PEAK_C)
        return (T - BODY_TEMP_C) / (THERMAL_MA_PEAK_C - BODY_TEMP_C);
      if (T <= THRESHOLDS.TEMP_WARN)
        return (
          (THRESHOLDS.TEMP_WARN - T) /
          (THRESHOLDS.TEMP_WARN - THERMAL_MA_PEAK_C)
        );
      return 0;
    },
    ...
    optimalFreqResult(): { khz: number; sel: number } {
      const state = this as CellStoreState;
      const target = state.target as CellConfig & {
        resonantFreqGHz?: number;
        resonantThresholdVcm?: number;
        capsidQ?: number;
      };
      if (
        ...
        const drH = (hVm * this.pulseEnvelopeFactorHealthy) / vthH;
        return {
          khz: freqKhz,
          sel: safeRatio(drT, drH, THRESHOLDS.TI_DISPLAY_CAP, NEAR_ZERO_DR),
        };
      }
      const sigma_e = this.effectiveSigmaE;
      // Apply same threshold corrections as the live DR getters so returned `sel` matches TI display.
      const hThr = effectiveElectroporationThreshold(
        state.healthy.thresholdVoltage,
        state.healthyTemp,
        state.waveform,
      );
      const tThr = effectiveElectroporationThreshold(
        state.target.thresholdVoltage,
        state.targetTemp,
        state.waveform,
      );
      ...
      const logMin = Math.log10(10),
        logMax = Math.log10(500_000);
      const { khz: optKhz, sel: maxSel } = Array.from(
        { length: 300 },
        (_, i) => {
          const khz = Math.pow(10, logMin + ((logMax - logMin) * i) / 299);
          const hDr =
            (computeSchwan(state.healthy, khz, UNIT_FIELD, sigma_e) * pefH) /
            hThr;
          const tDr =
            (computeSchwan(state.target, khz, UNIT_FIELD, sigma_e) * pefT) /
            tThr;
          return { khz, sel: hDr > 0 ? tDr / hDr : 0 };
        },
      ).reduce((best, pt) => (pt.sel > best.sel ? pt : best), {
        khz: 10,
        sel: -Infinity,
      });
    ...
    hmapFreqMaxKHz(): number {
      if (this.isResonanceMode) {
        ...
        if (t.resonantFreqGHz)
          return Math.max(t.resonantFreqGHz * 1e6 * 2.5, 1_000_000);
      }
      const cat = this.targetCellCategory;
      ...
      if (optKhz <= MAMMALIAN_DEFAULT || !isFinite(optKhz))
        return MAMMALIAN_DEFAULT;
      return Math.min(
        MAMMALIAN_CAP,
        Math.pow(10, Math.ceil(Math.log10(optKhz))),
      );
    },
    ...
    healthyBiomodScore(): number {
      ...
      if (this.healthyDisruptionRatio >= THRESHOLDS.HEALTHY_APPROACHING)
        return 0;
      return (
    ...
    isElectrodePolarizationRisk(): boolean {
      ...
      return (
        !this.isResonanceMode &&
        state.currentBroadcastFrequency < ELECTRODE_POLARIZATION_LIMIT_KHZ
      );
    },
    ...
    fcBelowSliderMin(): boolean {
      ...
      return (
        Math.min(this.healthyFc, this.targetFc) < this.sliderRanges.freqMin
      );
    },
    ...
    tiHighFreqLimit(): number {
      ...
      return (
        (state.target.radius * tauH * vthH) /
        (state.healthy.radius * tauT * vthT)
      );
    },
    ...
    sliderRanges(): SliderRange {
      ...
      if (this.isResonanceMode) {
        ...
        if (cat === CELL_CATEGORY.MAMMALIAN)
          return SLIDER_RANGES.RESONANCE_MAMMALIAN;
    ...
    targetPopulationLysisFraction(): number {
      return computePopulationLysisFraction(
        this.targetDisruptionRatio,
        this.targetPopulationSizeCV,
      );
    },
  ...
  actions: {
    ...
    setHealthyCellState(s: CellState) {
      this.healthyCellState = s;
    },
    setTargetCellState(s: CellState) {
      this.targetCellState = s;
    },
    ...
    startSession() {
      ...
      this.tempTimer = setInterval(() => {
        ...
        const hL =
          NEWTON_COOLING_LAMBDA +
          (this.perfusionRate * PENNES_BLOOD_COEFF) / hCp;
        const dTh =
          ((this.healthySAR * dc) / hCp -
            hL * (this.healthyTemp - BODY_TEMP_C)) *
          dt_s;
        this.healthyTemp = Math.max(
          BODY_TEMP_C,
          Math.min(THRESHOLDS.TEMP_CAP, this.healthyTemp + dTh),
        );
        const tCp = this.target.specificHeatCapacity;
        const tL =
          NEWTON_COOLING_LAMBDA +
          (this.perfusionRate * PENNES_BLOOD_COEFF) / tCp;
        const dTt =
          ((this.targetSAR * dc) / tCp - tL * (this.targetTemp - BODY_TEMP_C)) *
          dt_s;
        this.targetTemp = Math.max(
          BODY_TEMP_C,
          Math.min(THRESHOLDS.TEMP_CAP, this.targetTemp + dTt),
        );
      }, TEMP_UPDATE_INTERVAL_MS);
    },
    ...
    resetCell(cellType: "healthy" | "target") {
      const defaultCfg =
        cellType === CELL_TYPE.HEALTHY ? cellConfigs[0] : cellConfigs[1];

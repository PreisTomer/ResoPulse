    ...
    <table class="reports-log-table__table">
      <thead>
        <tr>
          ...
          <th
            class="reports-log-table__col-select"
            v-tip="$t('reports.colMethodsTitle')"
          >
            <svg
              class="reports-log-table__reticle-icon"
              viewBox="0 0 16 16"
              fill="none"
          ...
          <th v-for="col in TABLE_COLS" :key="col.key" v-tip="col.tip">
            {{ col.label }}
          </th>
        </tr>
      </thead>
        ...
        >
          <td class="reports-log-table__td-select">
            <span
              ...
              :class="{
                'reports-log-table__row-radio--on': selectedEntry?.id === e.id,
              }"
            ></span>
          </td>
          <td class="reports-log-table__session-val" v-tip="tipCellSession(e)">
          ...
          <td class="reports-log-table__cancer-val" v-tip="tipCellTargetVm(e)">
            {{ e.targetVm.toFixed(3) }}
          </td>
          <td class="reports-log-table__ref-val" v-tip="tipCellHealthyVm(e)">
            {{ e.healthyVm.toFixed(3) }}
          </td>
          <td :class="selClass(e.selectivity)" v-tip="tipCellSel(e)">
            {{ e.selectivity.toFixed(3) }}
          </td>
          <td
            :class="
              e.targetRatio >= THRESHOLDS.LYSIS_PROB_CENTER
                ? 'reports-log-table__cancer-val'
                : e.targetRatio >= THRESHOLDS.HEALTHY_APPROACHING
                  ? 'reports-log-table__warn-val'
                  : ''
            "
          ...
          <td class="reports-log-table__ref-val" v-tip="tipCellHRatio(e)">
            {{ (e.healthyRatio * 100).toFixed(1) }}%
          </td>
          <td
            :class="
              e.targetTemp > THRESHOLDS.TEMP_WARN
                ? 'reports-log-table__warn-val'
                : ''
            "
            v-tip="tipCellTemp(e.targetTemp, 'target')"
          >
            {{ e.targetTemp.toFixed(1) }}
          </td>
          <td
            :class="
              e.healthyTemp > THRESHOLDS.TEMP_WARN
                ? 'reports-log-table__warn-val'
                : ''
            "
            v-tip="tipCellTemp(e.healthyTemp, 'healthy')"
          >
            {{ e.healthyTemp.toFixed(1) }}
          </td>
          <td :class="depKClass(e.depHealthyK)" v-tip="tipCellDepH(e)">
            {{ depKDisplay(e.depHealthyK) }}
          </td>
          <td :class="depKClass(e.depTargetK)" v-tip="tipCellDepT(e)">
...
const TABLE_COLS_DEFS: Array<{
  ...
  {
    key: "session",
    labelKey: "reports.colSession",
    tipKey: "reports.colSessionTitle",
  },
  ...
  {
    key: "sel",
    labelKey: "reports.colSelectivity",
    tipKey: "reports.colSelectivityTitle",
  },
  {
    key: "tRatio",
    labelKey: "reports.colTRatio",
    tipKey: "reports.colTRatioTitle",
  },
  {
    key: "hRatio",
    labelKey: "reports.colHRatio",
    tipKey: "reports.colHRatioTitle",
  },
  {
    key: "tTemp",
    labelKey: "reports.colTTemp",
    tipKey: "reports.colTTempTitle",
  },
  {
    key: "hTemp",
    labelKey: "reports.colHTemp",
    tipKey: "reports.colHTempTitle",
  },
  { key: "depH", labelKey: "log.logThDepH", tipKey: "log.tipThDepH" },
  { key: "depT", labelKey: "log.logThDepT", tipKey: "log.tipThDepT" },
  {
    key: "event",
    labelKey: "reports.colEvent",
    tipKey: "reports.colEventTitle",
  },
];
      ...
      return this.$t("reports.tipCellTemp", {
        ...
        warn:
          temp > THRESHOLDS.TEMP_WARN
            ? ` ⚠ above ${THRESHOLDS.TEMP_WARN}°C hyperthermic limit`
            : "",
      });
    },
...
.reports-log-table {
  ...
  &__row-radio {
    ...
    &--on {
      ...
      box-shadow: 0 0 6px
        color-mix(in srgb, var(--color-purple) 55%, transparent);
    }
  }

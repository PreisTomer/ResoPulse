...
export function opentronsFilename(input: OpentronsExportInput): string {
  ...
  return `simbiotix_${target}_opentrons_${stamp}.py`
}
...
function buildDocstring(input: OpentronsExportInput, generatedAt: string): string {
  ...
  const lines: string[] = []
  lines.push('"""')
  lines.push('SimBiotix — Opentrons protocol export')
  lines.push(`Schema:    ${SCHEMA_VERSION}`)
  lines.push(`Generated: ${generatedAt}`)
  lines.push('Source:    SimBiotix closed-loop digital twin')
  lines.push('')
  ...
  lines.push('')
  lines.push('Wet-lab parameters (configured in SimBiotix export wizard):')
  ...
  lines.push('')
  lines.push('Generated by SimBiotix — Closed-Loop Electroporation Digital Twin.')
...
function buildMetadata(input: OpentronsExportInput): string {
  ...
  const protoName = pyString(`SimBiotix_${(input.cellPair.target.label || 'target').replace(/\s+/g, '_')}_${input.protocol.freqKHz}kHz_${input.wetLab.replicateCount}wells`)
  return [
    'metadata = {',
    `    "protocolName":   ${protoName},`,
    `    "author":         "SimBiotix digital twin",`,

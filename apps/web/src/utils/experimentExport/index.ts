...
export function buildEntryMethodsText(entry: LogEntry, sessionName: string, sampleDescription = ''): { text: string; filename: string } {
  ...
  if (!entry.healthySnap || !entry.targetSnap) {
    ...
    const text = [
      'MATERIALS AND METHODS: SimBiotix',
      sep('═'),
      ...
      '',
      sep('═'),
      'Generated by SimBiotix: Closed-Loop Electroporation Digital Twin',
      'Copyright © 2026 Tomer Preis. Licensed under the MIT License.',
    ].join('\n')
  ...
  const text = [
    'MATERIALS AND METHODS: SimBiotix Electroporation Simulation',
    sep('═'),
    `Session  : ${entrySession}`,
    `Entry    : #${entry.id}  logged at ${entry.timestamp}`,
    `Generated: ${new Date().toISOString()}`,
    ...
    '',
    sep('═'),
    'SimBiotix: virtual in-vitro biophysics engine',
    'Copyright © 2026 Tomer Preis. Licensed under the MIT License.',
  ].join('\n')
...
export function buildCsvText(
  ...
  const meta = [
    ...
    `# Model: Schwan equation (Kotnik & Miklavcic 2000), SimBiotix`,
    `# Copyright © 2026 Tomer Preis. Licensed under the MIT License.`,
  ].join('\n')
